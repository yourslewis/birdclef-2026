"""Guarded direct submitter for Itshyao S121 S114+G116 F1 delta.
...
an HGNetV2-B1 raw-pseudo G116 fold1 protected delta sidecar. The public dry-run
...
KERNEL_SLUG = "birdclef-2026-s121-s114-g116-f1-delta"
KERNEL_VERSION = 1
DESCRIPTION = "v587: Guarded direct Itshyao S121 S114 plus G116 F1 delta replay"

REQUIRED_SOURCE_MARKERS = [
    "S121: S114 anchor + G116 HGNetV2-B1 raw-pseudo fold1 protected delta",
    "S121_DELTA_WEIGHT = 0.035",
    "S121_MIN_TOP3_OVERLAP = 0.82",
    "S121_MIN_TOP10_OVERLAP = 0.88",
    "submission_g116_hgnet_b1_fold1_s121.csv",
    "S121 final submission",
    "test_soundscapes",
...
def output_preflight(kernels) -> bool:
    ...
    if "S121 final submission" not in log:
        print("Missing S121 final submission log marker", flush=True)
        return False
    if "S121 G116 fold1 sidecar failed" in log:
        print("S121 sidecar failed in public run; not submitting", flush=True)
        return False
    os.makedirs("logs", exist_ok=True)
    with open("logs/v587_itshyao_s121_g116_output_preflight.txt", "w") as f:
...
def main():
    ...
    if any(str(s.description) == DESCRIPTION for s in submissions):
        print("v587 already submitted; exiting", flush=True)
        return
