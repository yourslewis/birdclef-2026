"""Poll and submit v572 public946+cw0.75 local-window B0 sidecar kernel once ready.
...
KERNEL_SLUG = "bc26-v572-public946-cw075-b0-w00025"
KERNEL_VERSION = 1
DESCRIPTION = "v572: Public946 v542 plus cw0.75 local-window B0 rank sidecar 0.25%"
REQUIRED_FILES = {
    "submission.csv",
    "submission_cw075_localwindow_b0_student.csv",
    "submission_sed.csv",
    "submission_protossm.csv",
}
REQUIRED_LOG_MARKERS = [
    "cw0.75 local-window B0 student sidecar complete",
    "Applied cw0.75 local-window B0 student rank sidecar blend",
]
...
def main():
    ...
    while True:
        if DESCRIPTION in recent_descriptions(competitions):
            print("v572 already submitted; exiting.", flush=True)
            return
        ...
        if failure_message or "ERROR" in status_text or "FAILED" in status_text:
            raise SystemExit(f"v572 kernel failed; not submitting. failure={failure_message!r}")
        if "COMPLETE" not in status_text:
            print("v572 not complete yet; sleeping 10 minutes.", flush=True)
            time.sleep(600)
            continue
        if not output_ok(kernels):
            raise SystemExit("v572 COMPLETE but output verification failed; not submitting.")
        try:
            print(f"Submitting v572 kernel version {KERNEL_VERSION}...", flush=True)
