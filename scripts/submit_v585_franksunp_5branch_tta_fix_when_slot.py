"""Cap-aware submitter for FrankSunP 5-branch V4 TTA Fix.

Queued after v584 dropped to 0.942 and the 2026-05-19 UTC day was capped.
This is the best distinct next-reset candidate from the capped scan: full visible
source, COMPLETE/no-failure public run, and a valid sample-shaped output.
"""
...
KERNEL_OWNER = "franksunp"
KERNEL_SLUG = "birdclef-2026-5-branch-v4-tta-fix"
KERNEL_VERSION = 1
DESCRIPTION = "v585: Guarded direct FrankSunP 5-branch V4 TTA Fix replay"

REQUIRED_SOURCE_MARKERS = [
    "5-Branch Multistream Ensemble",
    "ProtoSSM",
    "Snowflake SED",
    "CLAP",
...
def output_preflight(kernels) -> bool:
    ...
    with open("logs/v585_franksunp_5branch_output_preflight.txt", "w") as f:
...
def main():
    ...
    while True:
        ...
        if any(str(s.description) == DESCRIPTION for s in submissions):
            print("v585 already submitted; exiting", flush=True)
            return
