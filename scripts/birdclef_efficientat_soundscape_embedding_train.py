#!/usr/bin/env python3
"""Train a bounded EfficientAT/AudioSet embedding-head BirdCLEF data point.

No-slot landscape probe for the ClawTeam hill-climb loop. It uses an
AudioSet-pretrained EfficientAT MobileNet/DyMN encoder on official
train_soundscape windows, then trains a small multilabel head for
non-Aves/no-train classes plus a no-call auxiliary target.
"""
...
@dataclass
class EfficientATSoundscapeConfig:
    experiment_id: str = "efficientat-mn10-audioset-soundscape-nonaves-notrain-nocall-siteS08-ep12-20260526"
    track: str = "EfficientAT AudioSet embedding non-Aves/no-train/no-call data point"
    data_root: str = "/home/yourslewis/birdclef-2026/data"
    output_dir: str = "artifacts/efficientat_soundscape_embeddings/efficientat-mn10-audioset-soundscape-nonaves-notrain-nocall-siteS08-ep12-20260526"
    efficientat_repo: str = "/home/yourslewis/external_models/EfficientAT"
...
def load_config(path: Path | None) -> EfficientATSoundscapeConfig:
    cfg = EfficientATSoundscapeConfig()
    if path is None:
        return cfg
    ...
    for key, value in data.items():
        if key in values:
            values[key] = value
    return EfficientATSoundscapeConfig(**values)
...
def decode_window(path: Path, start_sec: float, cfg: EfficientATSoundscapeConfig) -> np.ndarray:
...
def choose_labels(data_root: Path, cfg: EfficientATSoundscapeConfig, soundscape_df: pd.DataFrame) -> tuple[list[str], dict[str, Any]]:
...
def make_rows_and_targets(cfg: EfficientATSoundscapeConfig) -> tuple[list[dict[str, Any]], list[str], torch.Tensor, torch.Tensor, dict[str, Any]]:
...
def split_indices(rows: list[dict[str, Any]], cfg: EfficientATSoundscapeConfig) -> tuple[torch.Tensor, torch.Tensor, dict[str, Any]]:
...
def width_for_model(name: str) -> float:
    mn_map = {"mn01": 0.1, "mn02": 0.2, "mn04": 0.4, "mn05": 0.5, "mn06": 0.6, "mn08": 0.8,
              "mn10": 1.0, "mn12": 1.2, "mn14": 1.4, "mn16": 1.6, "mn20": 2.0, "mn30": 3.0, "mn40": 4.0}
    dymn_map = {"dymn04": 0.4, "dymn10": 1.0, "dymn20": 2.0}
    if name.startswith("dymn"):
        return dymn_map.get(name[:6], 1.0)
    return mn_map.get(name[:4], 1.0)


def load_efficientat(cfg: EfficientATSoundscapeConfig, device: torch.device):
    repo = Path(cfg.efficientat_repo)
    if not repo.exists():
        raise FileNotFoundError(f"EfficientAT repo not found: {repo}")
    sys.path.insert(0, str(repo))
    old_cwd = Path.cwd()
    os.chdir(repo)
    try:
        from models.preprocess import AugmentMelSTFT  # type: ignore
        if cfg.model_name.startswith("dymn"):
            from models.dymn.model import get_model  # type: ignore
        else:
            from models.mn.model import get_model  # type: ignore
        model = get_model(width_mult=width_for_model(cfg.model_name), pretrained_name=cfg.model_name)
        mel = AugmentMelSTFT(n_mels=cfg.n_mels, sr=cfg.sample_rate, win_length=cfg.window_size, hopsize=cfg.hop_size,
                             freqm=0, timem=0)
    finally:
        os.chdir(old_cwd)
...
def extract_efficientat_embeddings(rows: list[dict[str, Any]], cfg: EfficientATSoundscapeConfig, output_dir: Path) -> tuple[np.ndarray, np.ndarray, dict[str, Any]]:
    cache_path = output_dir / "efficientat_embeddings.npz"
    if cfg.cache_embeddings and cache_path.exists():
        obj = np.load(cache_path, allow_pickle=False)
        return obj["logits"], obj["embedding"], {"loaded_from_cache": True, "cache_path": str(cache_path)}

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model, mel = load_efficientat(cfg, device)
    t0 = time.time()
    ...
    for start in range(0, n, cfg.batch_size_extract):
        ...
        with torch.no_grad(), (torch.autocast(device_type=device.type, dtype=torch.float16) if use_amp else nullcontext()):
            spec = mel(waveform)
            logits, embedding = model(spec.unsqueeze(1))
        logits_batches.append(logits.float().detach().cpu().numpy().astype(np.float32))
        emb_batches.append(embedding.float().detach().cpu().numpy().astype(np.float32))
        print(f"[extract] {min(start + len(batch_rows), n)}/{n}", flush=True)
    logits_arr = np.concatenate(logits_batches, axis=0)
    ...
    meta = {
        ...
        "efficientat_repo": cfg.efficientat_repo,
    }
    if cfg.cache_embeddings:
        np.savez_compressed(cache_path, logits=logits_arr, embedding=embedding)
    return logits_arr, embedding, meta
...
def train_head(cfg: EfficientATSoundscapeConfig, embedding: np.ndarray, y: torch.Tensor, no_call: torch.Tensor, train_idx: torch.Tensor, val_idx: torch.Tensor, output_dir: Path) -> tuple[nn.Module, dict[str, Any], np.ndarray, np.ndarray]:
...
def main() -> None:
    ...
    audio_logits, embedding, extract_info = extract_efficientat_embeddings(rows, cfg, output_dir)
    _model, train_info, probs, no_call_probs = train_head(cfg, embedding, y, no_call, train_idx, val_idx, output_dir)
    ...
    metrics = {
        ...
        "efficientat_logits_mean": float(np.mean(audio_logits)),
        "efficientat_logits_std": float(np.std(audio_logits)),
        "prediction_stats": {
    ...
    print(json.dumps({
        ...
        "no_train_auc": metrics["macro_auc_no_train"]["macro_auc"],
