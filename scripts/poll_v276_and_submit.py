"""Poll v276 and submit completed kernel version to BirdCLEF 2026.
...
slug = "yourslewis/birdclef-2026-v276-immediate-protossm-ew055"
kernel_version = int(os.environ.get("KAGGLE_KERNEL_VERSION", "1"))
message = "v276: immediate-only temporal smoothing + ProtoSSM ensemble weight 0.55"
...
while True:
    ...
    if "COMPLETE" in status_text or "complete" in status_text.lower():
        try:
            res = api.competition_submit_code(file_name="submission.csv", message=message, competition="birdclef-2026", kernel=slug, kernel_version=kernel_version)
