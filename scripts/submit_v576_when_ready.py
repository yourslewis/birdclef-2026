"""Poll and submit v576 EoS5 Model5-only ablation once v575 confirms."""
...
KERNEL_SLUG = 'bc26-v576-eos5-model5-only-ablation'
KERNEL_VERSION = 1
DESCRIPTION = 'v576: Repo-owned EoS5 Model5-only ablation after v575 confirmation'
V575_DESCRIPTION = 'v575: Repo-owned EoS5 confirmation of v574 public949 path'
REQUIRED_FILES = {
    'submission.csv',
    'subm_5.csv',
...
def v575_confirmed(submissions) -> bool:
    for s in submissions:
        if str(s.description) == V575_DESCRIPTION:
            status = str(getattr(s, 'status', '')).lower()
            score = str(getattr(s, 'public_score', getattr(s, 'publicScore', '')))
            print(f'v575 status={status} score={score}', flush=True)
            return 'complete' in status and score in {'0.949', '0.950', '0.951'}
    print('v575 submission not visible yet', flush=True)
    return False
...
def output_ok(kernels) -> bool:
    ...
    with open('logs/v576_output_verification_summary.txt', 'w') as f:
...
def main():
    ...
    while True:
        ...
        if any(str(s.description) == DESCRIPTION for s in submissions):
            print('v576 already submitted; exiting.', flush=True)
            return
        ...
        if failure_message or 'ERROR' in status_text or 'FAILED' in status_text:
            raise SystemExit(f'v576 kernel failed; not submitting. failure={failure_message!r}')
        if 'COMPLETE' not in status_text:
            print('v576 not complete yet; sleeping 10 minutes.', flush=True)
            time.sleep(600)
            continue
        if not output_ok(kernels):
            raise SystemExit('v576 COMPLETE but output verification failed; not submitting.')
        submissions = recent_submissions(competitions)
        if not v575_confirmed(submissions):
            print('v575 has not confirmed 0.949+ yet; preserving slot and sleeping 10 minutes.', flush=True)
            time.sleep(600)
            continue
        try:
            print(f'Submitting v576 kernel version {KERNEL_VERSION}...', flush=True)
