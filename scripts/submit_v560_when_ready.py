"""Poll and submit v560 public946+direct V2S sidecar kernel once ready.
...
KERNEL_SLUG = "bc26-v560-public946-direct-v2s-r003"
KERNEL_VERSION = 1
...
def main():
    ...
    while True:
        if DESCRIPTION in recent_descriptions(competitions):
            print("v560 already submitted; exiting.", flush=True)
            return
        ...
        if "ERROR" in status_text or "FAILED" in status_text:
            raise SystemExit("v560 kernel failed; not submitting.")
        if "COMPLETE" not in status_text:
            print("v560 not complete yet; sleeping 10 minutes.", flush=True)
            time.sleep(600)
            continue
        if not has_submission_csv(kernels):
            raise SystemExit("v560 COMPLETE but submission.csv missing; not submitting.")
        try:
            print(f"Submitting v560 kernel version {KERNEL_VERSION}...", flush=True)
