"""Guarded submitter for v598 Samejima HGNet OpenVINO artifact port.

v598 is a repo-owned guarded notebook built from Samejima's public HGNetV2-B0
baseline inference source plus the completed/errored public training kernel's
artifact files. It fixes the public dry-run row-id/NaN merge path and attaches
Samejima training outputs as a kernel source so OpenVINO XML/BIN weights are
available without rebuilding them.
"""
...
KERNEL_SLUG = "bc26-v598-samejima-hgnet-openvino-artifact"
KERNEL_VERSION = 1
DESCRIPTION = "v598: Repo-owned Samejima HGNet OpenVINO artifact inference"

REQUIRED_SOURCE_MARKERS = [
    "v598 SAMEJIMA_TRAINED_MODEL",
    "birdclef-2026-hgnetv2-b0-baseline-training",
    "v598 dry-run sample_sub realigned",
    "v598 missing values after sample merge",
    "v598 submission.csv written",
    "USE_OPENVINO = True",
    "best_model_fold{fold_id}_256x512.xml",
    "test_soundscapes",
    "train_soundscapes",
...
def output_preflight(kernels) -> bool:
    ...
    with open("logs/v598_samejima_hgnet_openvino_preflight.txt", "w") as f:
...
def main():
    ...
    parser.add_argument("--wait-for-slot", action="store_true", help="Sleep until UTC reset if current daily cap is exhausted.")
    ...
    while True:
        ...
        if any(str(s.description) == DESCRIPTION for s in submissions):
            print("v598 already submitted; exiting", flush=True)
            return
        ...
        if today_count >= 5:
            ...
            print(f"daily cap appears exhausted; sleeping {sleep_s}s until after UTC reset", flush=True)
            time.sleep(sleep_s)
            continue
        try:
            ...
            if args.wait_for_slot and ("daily Submission allowance" in text or ("daily" in text.lower() and "allowance" in text.lower())):
