"""Guarded fallback submitter for Amulopapa Youssef+gate rank blend.

Waits for v581 A2Prime/NFNet to score first. If v581 improves beyond the
current 0.949 best, this exits so follow-up can focus on confirmation. If v581
ties/drops/no-scores, this can use a later slot for a distinct Youssef+gate
frontier hypothesis.
"""
...
KERNEL_OWNER = "amulopapa67"
KERNEL_SLUG = "bc26-full-yous-gate-rb035-nb-20260517"
KERNEL_VERSION = 4
DESCRIPTION = "v582: Guarded direct Amulopapa Youssef gate rb035 frontier replay"
V581_DESCRIPTION = "v581: Guarded direct Lucataco A2Prime NFNet frontier replay"
CURRENT_BEST = 0.949

REQUIRED_SOURCE_MARKERS = [
    "submission_youssef.csv",
    "submission_gate.csv",
    "0.65 * yr",
    "0.35 * gr",
    "submission.csv",
...
def v581_allows_fallback(submissions) -> bool:
    for sub in submissions:
        if str(sub.description) == V581_DESCRIPTION:
            status = str(getattr(sub, "status", "")).lower()
            score = score_float(sub)
            print(f"v581 visible: status={status} score={score}", flush=True)
            if "complete" not in status:
                return False
            if score is not None and score > CURRENT_BEST:
                raise SystemExit(f"v581 improved to {score}; stop v582 and port/confirm v581 instead")
            return True
    print("v581 not visible yet; preserving slots", flush=True)
    return False
...
def output_preflight(kernels) -> bool:
    ...
    with open("logs/v582_amulopapa_yous_gate_output_preflight.txt", "w") as f:
...
def main():
    ...
    while True:
        ...
        if any(str(s.description) == DESCRIPTION for s in submissions):
            print("v582 already submitted; exiting", flush=True)
            return
        if not v581_allows_fallback(submissions):
            print("waiting for v581 result before v582 fallback; sleeping 15 minutes", flush=True)
            time.sleep(900)
            continue
