"""Poll and submit v577 EoS5 rank-power 0.55 ablation after v576 confirms."""
...
KERNEL_SLUG = 'bc26-v577-eos5-model5-rank-power-0-55'
KERNEL_VERSION = 1
DESCRIPTION = 'v577: Repo-owned EoS5 Model5-only rank-aware power 0.55'
V576_DESCRIPTION = 'v576: Repo-owned EoS5 Model5-only ablation after v575 confirmation'
REQUIRED_FILES = {
    'submission.csv',
    'subm_5.csv',
...
def prerequisite_confirmed(submissions) -> bool:
    for s in submissions:
        if str(s.description) == V576_DESCRIPTION:
            status = str(getattr(s, 'status', '')).lower()
            score = str(getattr(s, 'public_score', getattr(s, 'publicScore', '')))
            print(f'v576 status={status} score={score}', flush=True)
            return 'complete' in status and score in {'0.949', '0.950', '0.951'}
    print('v576 submission not visible yet', flush=True)
    return False
...
def output_ok(kernels) -> bool:
    ...
    forbidden_present = sorted(FORBIDDEN_FILES & files)
    ...
    if forbidden_present:
        print(f'Forbidden files unexpectedly present: {forbidden_present}', flush=True)
    if missing_markers:
        print(f'Missing required log markers: {missing_markers}', flush=True)
    os.makedirs('logs', exist_ok=True)
    with open('logs/v577_output_verification_summary.txt', 'w') as f:
        f.write('files=' + repr(sorted(files)) + '\n')
        f.write('missing_files=' + repr(missing_files) + '\n')
        f.write('forbidden_present=' + repr(forbidden_present) + '\n')
        f.write('missing_markers=' + repr(missing_markers) + '\n')
        f.write('log_tail=' + log[-4000:] + '\n')
    return not missing_files and not forbidden_present and not missing_markers
...
def main():
    ...
    while True:
        ...
        if any(str(s.description) == DESCRIPTION for s in submissions):
            print('v577 already submitted; exiting.', flush=True)
            return
        ...
        if failure_message or 'ERROR' in status_text or 'FAILED' in status_text:
            raise SystemExit(f'v577 kernel failed; not submitting. failure={failure_message!r}')
        if 'COMPLETE' not in status_text:
            print('v577 not complete yet; sleeping 10 minutes.', flush=True)
            time.sleep(600)
            continue
        if not output_ok(kernels):
            raise SystemExit('v577 COMPLETE but output verification failed; not submitting.')
        submissions = recent_submissions(competitions)
        if not prerequisite_confirmed(submissions):
            print('v576 has not confirmed 0.949+ yet; preserving slot and sleeping 10 minutes.', flush=True)
            time.sleep(600)
            continue
        try:
            print(f'Submitting v577 kernel version {KERNEL_VERSION}...', flush=True)
