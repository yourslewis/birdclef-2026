"""Guarded submitter for v612 anchored Samejima visual + HGNet-v57 PT sidecar."""
...
KERNEL_SLUG = "bc26-v612-anchored-sameji-hgnet57-pt"
KERNEL_VERSION = 1
DESCRIPTION = "v612: Repo-owned Samejima anchor plus HGNet-v57 PT sidecar"
TODAY_UTC = "2026-05-24"
REQUIRED_OUTPUTS = {
    "submission.csv",
    "submission_anchor_raw.csv",
    "submission_sameji_hgnet57_raw.csv",
...
def call(label: str, fn: Callable[[], T], attempts: int = 4, sleep_s: int = 30) -> T:
...
def output_preflight(kernels) -> bool:
    ...
    if "Traceback" in log or "NotebookThrewException" in log:
        ...
        return False
    for marker in [
        "v612 preserved Samejima visual anchor",
        "v612 wrote submission_sameji_hgnet57_raw.csv",
        "v612 anchored rank blend",
        "v612 wrote submission.csv",
    ]:
    ...
    anchor = stats["submission_anchor_raw.csv"]
    sidecar = stats["submission_sameji_hgnet57_raw.csv"]
    for label, st in [("final", final), ("anchor", anchor), ("sidecar", sidecar)]:
        if st["n_cols"] != 235 or st["bad_values"] or st["unique_row_ids"] != st["n_rows"]:
            print("bad stats", label, st, flush=True)
            return False
    if final["unique_rounded_first_10000"] <= 1000:
        print("final appears too low-diversity", final, flush=True)
        return False
    if sidecar["unique_rounded_first_10000"] <= 1000:
        print("sidecar appears too low-diversity", sidecar, flush=True)
        return False
    os.makedirs("logs", exist_ok=True)
    with open("logs/v612_anchored_sameji_hgnet57_preflight.txt", "w") as f:
...
def main():
    ...
    if any(str(getattr(s, "description", "")) == DESCRIPTION for s in submissions):
        print("v612 already submitted; exiting", flush=True)
        return
    if today_count >= 5:
