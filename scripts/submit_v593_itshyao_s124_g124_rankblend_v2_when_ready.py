"""Guarded submitter for v593 Itshyao S124/S114+G124 F1 RankBlend v2.

Prepared after user reported the public post is now 0.952. We previously submitted
v589 against kernel version 1 and got 0.949. Current Kaggle metadata exposes version
2. Even though decoded source appears unchanged, the independent 0.952 signal makes
v2 a better reset-slot owner than v592 HGNet until proven otherwise.
"""
...
KERNEL_VERSION = 2
DESCRIPTION = "v593: Guarded direct Itshyao S124 S114 plus G124 F1 rankblend v2 0952 lead"
...
def output_preflight(kernels) -> bool:
    ...
    with open("logs/v593_itshyao_s124_g124_v2_preflight.txt", "w") as f:
...
def main():
    ...
    if any(str(s.description) == DESCRIPTION for s in submissions):
        print("v593 already submitted; exiting", flush=True)
        return
