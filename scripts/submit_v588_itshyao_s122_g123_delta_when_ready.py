"""Guarded direct submitter for Itshyao S122 S114+G123 F1 delta.
...
an EfficientNetV2-B0 pseudo G123 fold1 protected delta sidecar. The public dry-run
...
KERNEL_SLUG = "birdclef-2026-s122-s114-g123-f1-delta"
KERNEL_VERSION = 1
DESCRIPTION = "v588: Guarded direct Itshyao S122 S114 plus G123 F1 delta replay"

REQUIRED_SOURCE_MARKERS = [
    "S122: S114 anchor + G123 EfficientNetV2-B0 pseudo fold1 protected delta",
    "S122_DELTA_WEIGHT = 0.045",
    "S122_MIN_TOP3_OVERLAP = 0.80",
    "S122_MIN_TOP10_OVERLAP = 0.86",
    "submission_g123_effv2b0_fold1_s122.csv",
    "S122 final submission",
    "test_soundscapes",
...
def output_preflight(kernels) -> bool:
    ...
    if "S122 final submission" not in log:
        print("Missing S122 final submission log marker", flush=True)
        return False
    if "S122 G123 fold1 sidecar failed" in log:
        print("S122 sidecar failed in public run; not submitting", flush=True)
        return False
    os.makedirs("logs", exist_ok=True)
    with open("logs/v588_itshyao_s122_g123_output_preflight.txt", "w") as f:
...
def main():
    ...
    if any(str(s.description) == DESCRIPTION for s in submissions):
        print("v588 already submitted; exiting", flush=True)
        return
