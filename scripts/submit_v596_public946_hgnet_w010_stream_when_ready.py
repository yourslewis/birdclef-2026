"""Guarded submitter for v596 public946 + Qiuzi HGNet 10% sidecar streaming repair.

Prepared while 2026-05-20 UTC slots were capped. This is a repo-owned,
source-validated structural diagnostic: public946 Proto/SED anchor plus the
complete Qiuzi 4-fold HGNetV2-B0 distill artifact as a 10% rank sidecar, refactored to stream one file/fold at a time after v594 RAM failure.
"""
...
KERNEL_SLUG = "bc26-v596-public946-hgnet-w010-stream"
KERNEL_VERSION = 1
DESCRIPTION = "v596: Repo-owned public946 plus streaming Qiuzi HGNet rank sidecar 10pct"

# Markers chosen to distinguish this from generic EoS/Karnak/S114 clones.
REQUIRED_SOURCE_MARKERS = [
    "HGNET_RANK_WEIGHT = 0.10",
    "submission_hgnet.csv",
    "Executing v596 streaming 3-way rank blend",
    "v596 streaming HGNet audio groups",
    "_hgnet_probs_sum",
    "_load_audio_once",
    "_segment_from_wav",
    "RSS=",
...
def output_preflight(kernels) -> bool:
    ...
    with open("logs/v596_public946_hgnet_w010_stream_preflight.txt", "w") as f:
...
def main():
    ...
    if any(str(s.description) == DESCRIPTION for s in submissions):
        print("v596 already submitted; exiting", flush=True)
        return
