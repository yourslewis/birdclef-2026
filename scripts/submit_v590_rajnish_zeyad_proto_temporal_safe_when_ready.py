"""Guarded direct submitter for Rajnish/Zeyad Proto Temporal Safe.

Prepared while 2026-05-20 UTC slots were capped after v585-v589. This is a
non-S114 backup candidate: exp019/Pilkwang-style 0.948/0.949 Proto+SED rank
path plus small independent BirdNET / centroid / public-CNN branches and
Temporal flip/shift TTA. It is not a 0.96-first candidate, but it is cleaner
and more structurally distinct than more S114+G sidecar siblings if a reset
slot would otherwise idle.
"""
...
KERNEL_OWNER = "rajnish1419kumar"
KERNEL_SLUG = "birdclef-2026-zeyad-proto-temporal-safe"
KERNEL_VERSION = 1
DESCRIPTION = "v590: Guarded direct Rajnish Zeyad Proto Temporal Safe replay"

# Markers chosen to distinguish this from generic EoS/Karnak/S114 clones.
REQUIRED_SOURCE_MARKERS = [
    "consolidated 0.949 inference script",
    "Temporal-shift TTA",
    "Temporal flip as extra TTA pass",
    "ENABLE_BIRDNET_BRANCH = True",
    "ENABLE_CENTROID_BRANCH = True",
    "ENABLE_PUBLIC_CNN_BRANCH = True",
    "FINAL_BLEND_PRESET = \"conservative\"",
    '"conservative":  {"proto": 0.54, "sed": 0.34',
    "submission_birdnet.csv",
    "submission_centroid.csv",
    "submission_public_cnn.csv",
...
def source_preflight(token: str) -> bool:
    ...
    # Kaggle pull returns notebook JSON for notebooks. Decode cells before marker
    # matching so quotes/newlines are checked against actual Python source rather
    # than JSON-escaped text.
...
def output_preflight(kernels) -> bool:
    ...
    with open("logs/v590_rajnish_zeyad_proto_temporal_preflight.txt", "w") as f:
...
def main():
    ...
    if any(str(s.description) == DESCRIPTION for s in submissions):
        print("v590 already submitted; exiting", flush=True)
        return
