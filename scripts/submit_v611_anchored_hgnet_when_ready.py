"""Guarded submitter for v611 anchored Samejima + Praxel HGNet sidecar."""
...
KERNEL_SLUG = "bc26-v611-anchored-hgnet-sidecar"
KERNEL_VERSION = 1
DESCRIPTION = "v611: Repo-owned Samejima anchor plus Praxel HGNet sidecar"
...
def output_preflight(kernels) -> bool:
    ...
    if "Traceback" in log or "NotebookThrewException" in log:
        ...
        return False
    for marker in [
        "v611 preserved Samejima anchor",
        "v611 wrote submission_prax_hgnet_raw.csv",
        "v611 anchored rank blend",
        "v611 wrote submission.csv",
    ]:
    ...
    sidecar = stats["submission_prax_hgnet_raw.csv"]
    ...
    with open("logs/v611_anchored_hgnet_preflight.txt", "w") as f:
...
def main():
    ...
    if any(str(getattr(s, "description", "")) == DESCRIPTION for s in submissions):
        print("v611 already submitted; exiting", flush=True)
        return
    if today_count >= 5:
