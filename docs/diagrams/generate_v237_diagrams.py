#!/usr/bin/env python3
"""Generate BirdCLEF v237 workflow and model-architecture diagrams.
...
parsed from kaggle-kernels/v237-weighted-ensemble/script.py so labels stay useful
as v237 constants change.
"""
...
DEFAULT_SCRIPT = REPO_ROOT / "kaggle-kernels" / "v237-weighted-ensemble" / "script.py"
...
def build_workflow(facts: ScriptFacts) -> Diagram:
    ...
    return Diagram(
        name="birdclef_v237_workflow",
        title="BirdCLEF v237 Weighted-Ensemble Workflow",
        subtitle="End-to-end path from Kaggle inputs to submission.csv",
        width=1260,
        height=900,
        nodes=nodes,
        edges=edges,
        notes=("Generated from docs/diagrams/generate_v237_diagrams.py", "Target: kaggle-kernels/v237-weighted-ensemble/script.py"),
    )
...
def build_architecture(facts: ScriptFacts) -> Diagram:
    ...
    return Diagram(
        name="birdclef_v237_architecture",
        title="BirdCLEF v237 Model Architecture",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate BirdCLEF v237 workflow/model diagrams.")
    parser.add_argument("--script", type=Path, default=DEFAULT_SCRIPT, help="Path to v237 script.py")
