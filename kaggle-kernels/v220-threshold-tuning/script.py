...
def get_dynamic_thresholds(class_frequencies, base_thresh=0.4, rare_boost=0.15):
    # class_frequencies: array or list of how many times each class appears
    # Rare classes get a lower threshold to improve recall
    # Common classes get a higher threshold to improve precision
    max_freq = np.max(class_frequencies)
    thresholds = []
    for freq in class_frequencies:
        # Scale between base_thresh - rare_boost and base_thresh + rare_boost
        ratio = np.log1p(freq) / np.log1p(max_freq)
        thresh = (base_thresh - rare_boost) + (ratio * (rare_boost * 2))
