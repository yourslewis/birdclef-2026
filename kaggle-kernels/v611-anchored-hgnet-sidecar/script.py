#!/usr/bin/env python3
"""
BirdCLEF 2026 v611 anchored HGNet sidecar scaffold.
...
Sidecar: Praxel/Kosuke HGNetV2-B0 OpenVINO raw branch, reimplemented as a memory-streaming hidden-test rerun.
Final: fixed low-weight columnwise rank blend: 0.94 anchor + 0.06 Praxel HGNet raw.
...
# NOTE: generated scaffold: Samejima cells 1-14 are preserved below, then v611
# injects a Praxel HGNet OpenVINO sidecar and overwrites submission.csv.
...
# %% v611 Praxel/Kosuke HGNet OpenVINO sidecar and anchored rank blend
# The Samejima anchor has just written submission.csv. Preserve it before adding
# the low-weight sidecar, then hard-fail on any missing model/artifact/alignment.
import subprocess as _v611_subprocess
import sys as _v611_sys
import gc as _v611_gc
import math as _v611_math
from pathlib import Path as _V611Path
from time import time as _v611_time

import numpy as _v611_np
import pandas as _v611_pd
import soundfile as _v611_soundfile
import torch as _v611_torch
from torch import nn as _v611_nn

ANCHOR_RAW_CSV = "submission_anchor_raw.csv"
PRAX_HGNET_RAW_CSV = "submission_prax_hgnet_raw.csv"
BEFORE_ALIGNMENT_CSV = "submission_before_alignment.csv"
FINAL_CSV = "submission.csv"
ANCHOR_RANK_WEIGHT = 0.94
PRAX_HGNET_RANK_WEIGHT = 0.06

_anchor_df = _v611_pd.read_csv(FINAL_CSV)
_anchor_cols = [c for c in _anchor_df.columns if c != "row_id"]
assert _anchor_cols, "Samejima anchor produced no class columns"
assert _anchor_df["row_id"].is_unique, "Samejima anchor row_id values are not unique"
_anchor_vals = _anchor_df[_anchor_cols].to_numpy(_v611_np.float32)
assert _v611_np.isfinite(_anchor_vals).all(), "Samejima anchor contains non-finite values"
assert _anchor_vals.min() >= 0.0 and _anchor_vals.max() <= 1.0, "Samejima anchor outside [0,1]"
_anchor_df.to_csv(ANCHOR_RAW_CSV, index=False)
print(f"v611 preserved Samejima anchor as {ANCHOR_RAW_CSV}: shape={_anchor_df.shape}")

# Robust offline install for Praxel/TTAhara HGNet OpenVINO wheels. The wheel
# notebook can be mounted under several /kaggle/input paths, so discover it.
_v611_req_candidates = []
for _req in _V611Path("/kaggle/input").rglob("requirements.txt"):
    _wheel_dir = _req.parent / "wheels"
    if _wheel_dir.exists():
        _txt = _req.read_text(errors="ignore").lower()
        if "openvino" in _txt and "onnxruntime" in _txt:
            _v611_req_candidates.append(_req)
if not _v611_req_candidates:
    _shown = [str(p) for p in _V611Path("/kaggle/input").rglob("requirements.txt")][:20]
    raise FileNotFoundError(
        "Could not find offline HGNet/OpenVINO wheel requirements under /kaggle/input. "
        f"Available requirements samples: {_shown}"
    )
_v611_req = sorted(_v611_req_candidates, key=lambda p: len(str(p)))[0]
_v611_wheel_dir = _v611_req.parent / "wheels"
print(f"v611 installing HGNet/OpenVINO wheels from {_v611_req}")
_v611_subprocess.run(
    [
        _v611_sys.executable,
        "-m",
        "pip",
        "install",
        "-q",
...
import torchaudio as _v611_torchaudio
from torchvision.transforms import v2 as _v611_tvt_v2
import openvino as _v611_ov

# Locate Praxel/TTAhara OpenVINO HGNet fold artifacts. Praxel's public source
# searches for best_model_fold0.xml; use the same contract and require all folds.
_v611_model_hits = sorted(_V611Path("/kaggle/input").rglob("best_model_fold0.xml"))
if not _v611_model_hits:
    _shown = [str(p) for p in _V611Path("/kaggle/input").rglob("best_model_fold0.*")][:20]
    raise FileNotFoundError(
        "Could not find Praxel/TTAhara HGNet OpenVINO best_model_fold0.xml. "
        f"Available best_model_fold0 samples: {_shown}"
    )
PRAX_HGNET_MODEL_DIR = _v611_model_hits[0].parent
N_FOLDS_PRAX = 4
for _fold in range(N_FOLDS_PRAX):
    _xml = PRAX_HGNET_MODEL_DIR / f"best_model_fold{_fold}.xml"
    _bin = PRAX_HGNET_MODEL_DIR / f"best_model_fold{_fold}.bin"
    if not _xml.exists():
        raise FileNotFoundError(f"Missing Praxel HGNet fold XML: {_xml}")
    if not _bin.exists():
        raise FileNotFoundError(f"Missing Praxel HGNet fold BIN: {_bin}")
print(f"v611 Praxel HGNet OpenVINO model dir: {PRAX_HGNET_MODEL_DIR}")

# Map OpenVINO model output columns (Praxel uses taxonomy.primary_label order) to
# the anchor/sample column order. Hard-fail if schemas diverge.
_v611_taxonomy = _v611_pd.read_csv(BASE / "taxonomy.csv")
_v611_model_cols = _v611_taxonomy["primary_label"].astype(str).tolist()
if set(_v611_model_cols) != set(_anchor_cols):
    raise RuntimeError(
        f"Praxel HGNet taxonomy columns differ from anchor columns: "
        f"model_only={sorted(set(_v611_model_cols)-set(_anchor_cols))[:5]}, "
        f"anchor_only={sorted(set(_anchor_cols)-set(_v611_model_cols))[:5]}"
    )
_v611_model_col_index = _v611_np.array([_v611_model_cols.index(c) for c in _anchor_cols], dtype=_v611_np.int64)

PRAX_SR = 32_000
PRAX_SEGMENT_SEC = 5
PRAX_FILE_SEC = 60
PRAX_SHIFT_SEC = 2.5
PRAX_SHIFT = int(PRAX_SR * PRAX_SHIFT_SEC)
PRAX_DURATION = int(PRAX_SR * PRAX_FILE_SEC)
PRAX_BATCH = 12
PRAX_N_CLASSES = len(_anchor_cols)

class _V611LogMelSpectrogramTransform(_v611_nn.Module):
    def __init__(self):
        super().__init__()
        self.mel_transform = _v611_torchaudio.transforms.MelSpectrogram(
            sample_rate=PRAX_SR,
            n_fft=2048,
            win_length=626,
            hop_length=313,
        ...
        self.db = _v611_torchaudio.transforms.AmplitudeToDB(stype="power", top_db=80.0)
        self.resize = _v611_tvt_v2.Resize(size=(256, 256))
...
def _v611_load_padded_wave(audio_id: str):
    path = _v611_audio_path(audio_id)
    y, sr0 = _v611_soundfile.read(str(path), dtype="float32", always_2d=False)
    if getattr(y, "ndim", 1) == 2:
        y = y.mean(axis=1)
    y = y.astype(_v611_np.float32, copy=False)
    if int(sr0) != PRAX_SR:
        y = _v611_torchaudio.functional.resample(
            _v611_torch.from_numpy(y), int(sr0), PRAX_SR
        ).numpy().astype(_v611_np.float32)
    # Praxel/TTAhara TTA contract: a 2.5s left pad and 2.5s right pad around a
    # 60s clip, with shifted windows sampled from the unshifted padded array.
    padded = _v611_np.zeros(PRAX_SHIFT + PRAX_DURATION + PRAX_SHIFT, dtype=_v611_np.float32)
    n = min(len(y), PRAX_DURATION + PRAX_SHIFT)
    padded[PRAX_SHIFT : PRAX_SHIFT + n] = y[:n]
    return padded

def _v611_segment_triplet(padded: _v611_np.ndarray, end_sec: int):
    if end_sec % PRAX_SEGMENT_SEC != 0 or end_sec < PRAX_SEGMENT_SEC or end_sec > PRAX_FILE_SEC:
        raise ValueError(f"Unexpected BirdCLEF end_sec for Praxel HGNet: {end_sec}")
    center_start = PRAX_SHIFT + int((end_sec - PRAX_SEGMENT_SEC) * PRAX_SR)
    center_stop = PRAX_SHIFT + int(end_sec * PRAX_SR)
    prev_start = int((end_sec - PRAX_SEGMENT_SEC) * PRAX_SR)
    prev_stop = int(end_sec * PRAX_SR)
    next_start = int(end_sec * PRAX_SR)
    next_stop = int((end_sec + PRAX_SEGMENT_SEC) * PRAX_SR)
    return (
        padded[center_start:center_stop],
        padded[prev_start:prev_stop],
        padded[next_start:next_stop],
    )

def _v611_compile_ov_model(xml_path: _V611Path):
    core = _v611_ov.Core()
    return core.compile_model(
        str(xml_path),
        "CPU",
        {"PERFORMANCE_HINT": "THROUGHPUT", "INFERENCE_NUM_THREADS": 4, "NUM_STREAMS": 2},
    )

def _v611_ov_predict(compiled_model, segments, lms_transform):
    if not segments:
        return _v611_np.zeros((0, PRAX_N_CLASSES), dtype=_v611_np.float32)
    input_name = compiled_model.inputs[0].get_any_name()
    out_batches = []
    with _v611_torch.no_grad():
        for start in range(0, len(segments), PRAX_BATCH):
            batch = _v611_np.stack(segments[start : start + PRAX_BATCH]).astype(_v611_np.float32, copy=False)
            lms = lms_transform(_v611_torch.from_numpy(batch)).numpy().astype(_v611_np.float32, copy=False)
            result = compiled_model({input_name: lms})
            logits = next(iter(result.values()))
            out_batches.append(_v611_np.asarray(logits, dtype=_v611_np.float32)[:, _v611_model_col_index])
            del batch, lms, result, logits
    return _v611_np.concatenate(out_batches, axis=0)

def _v611_rank_normalize(x: _v611_np.ndarray) -> _v611_np.ndarray:
    return _v611_pd.DataFrame(x).rank(axis=0, method="average", pct=True).to_numpy(_v611_np.float32)

# Build target groups from the anchor rows so the sidecar is guaranteed to rerun
# on exactly the same row IDs as the Samejima/v608-family anchor.
_v611_row_ids = _anchor_df["row_id"].astype(str).tolist()
_v611_groups = {}
for _row_pos, _rid in enumerate(_v611_row_ids):
    _aid, _end_sec = _v611_parse_row_id(_rid)
    _v611_groups.setdefault(_aid, []).append((_row_pos, _end_sec))
print(
    f"v611 Praxel HGNet target rows={len(_v611_row_ids)}, "
    f"audio_files={len(_v611_groups)}, hidden_test={len(list((BASE / 'test_soundscapes').glob('*.ogg'))) > 0}"
)

_lms_transform = _V611LogMelSpectrogramTransform().eval()
_prax_probs_sum = _v611_np.zeros((len(_v611_row_ids), PRAX_N_CLASSES), dtype=_v611_np.float32)

for _fold in range(N_FOLDS_PRAX):
    _fold_t0 = _v611_time()
    _compiled = _v611_compile_ov_model(PRAX_HGNET_MODEL_DIR / f"best_model_fold{_fold}.xml")
    _fold_min = float("inf")
    _fold_max = float("-inf")
    _fold_rows = 0
    for _audio_i, (_aid, _items) in enumerate(_v611_groups.items(), 1):
        _padded = _v611_load_padded_wave(_aid)
        ...
        for _, _end_sec in _items_sorted:
            _center, _prev, _next = _v611_segment_triplet(_padded, _end_sec)
            _center_segments.append(_center)
            _prev_segments.append(_prev)
            _next_segments.append(_next)
        _center_logits = _v611_ov_predict(_compiled, _center_segments, _lms_transform)
        _prev_logits = _v611_ov_predict(_compiled, _prev_segments, _lms_transform)
        _next_logits = _v611_ov_predict(_compiled, _next_segments, _lms_transform)
        _logits = 0.50 * _center_logits + 0.25 * _prev_logits + 0.25 * _next_logits
        _probs = (1.0 / (1.0 + _v611_np.exp(-_logits))).astype(_v611_np.float32)
        _prax_probs_sum[_idxs, :] += _probs / float(N_FOLDS_PRAX)
        _fold_min = min(_fold_min, float(_v611_np.nanmin(_probs)))
        _fold_max = max(_fold_max, float(_v611_np.nanmax(_probs)))
        ...
        if _audio_i % 50 == 0:
            print(f"v611 Praxel HGNet fold {_fold}: audio_groups={_audio_i}/{len(_v611_groups)}")
    print(
        f"v611 Praxel HGNet fold {_fold}: rows={_fold_rows}, "
        f"prob_min={_fold_min:.6f}, prob_max={_fold_max:.6f}, time={_v611_time()-_fold_t0:.1f}s"
    )
    del _compiled
    _v611_gc.collect()

_prax_probs = _prax_probs_sum.astype(_v611_np.float32)
if _prax_probs.shape != (len(_v611_row_ids), len(_anchor_cols)):
    raise RuntimeError(f"Praxel HGNet shape mismatch: {_prax_probs.shape} vs {(len(_v611_row_ids), len(_anchor_cols))}")
if not _v611_np.isfinite(_prax_probs).all():
    raise RuntimeError("Praxel HGNet predictions contain non-finite values")
if float(_v611_np.max(_prax_probs) - _v611_np.min(_prax_probs)) <= 1e-8:
    raise RuntimeError("Praxel HGNet predictions are constant; refusing silent fallback")

_prax_df = _anchor_df[["row_id"]].copy()
_prax_df[_anchor_cols] = _prax_probs
_prax_df.to_csv(PRAX_HGNET_RAW_CSV, index=False)
print(
    f"v611 wrote {PRAX_HGNET_RAW_CSV}: shape={_prax_df.shape}, "
    f"min={float(_prax_probs.min()):.6f}, max={float(_prax_probs.max()):.6f}"
)

_anchor_arr = _v611_np.clip(_anchor_df[_anchor_cols].to_numpy(_v611_np.float32), 1e-6, 1.0 - 1e-6)
_prax_arr = _v611_np.clip(_prax_df[_anchor_cols].to_numpy(_v611_np.float32), 1e-6, 1.0 - 1e-6)
_anchor_rank = _v611_rank_normalize(_anchor_arr)
_prax_rank = _v611_rank_normalize(_prax_arr)
_final_pred = ANCHOR_RANK_WEIGHT * _anchor_rank + PRAX_HGNET_RANK_WEIGHT * _prax_rank
print(
    f"v611 anchored rank blend: anchor={ANCHOR_RANK_WEIGHT:.3f}, "
    f"prax_hgnet={PRAX_HGNET_RANK_WEIGHT:.3f}, "
    f"corr={_v611_np.corrcoef(_anchor_rank.ravel(), _prax_rank.ravel())[0, 1]:.6f}, "
    f"mae={float(_v611_np.mean(_v611_np.abs(_anchor_rank - _prax_rank))):.6f}"
)

_final_df = _anchor_df.copy()
_final_df[_anchor_cols] = _final_pred.astype(_v611_np.float32)
_final_df.to_csv(BEFORE_ALIGNMENT_CSV, index=False)

# Hidden-safe final guard. We deliberately keep train-soundscape dry-run row IDs
# instead of writing a fixed/constant sample fallback; hidden submissions naturally
# use test_soundscapes row IDs from the anchor/sidecar rerun.
assert list(_final_df.columns) == ["row_id"] + _anchor_cols
assert _final_df["row_id"].is_unique
_final_vals = _final_df[_anchor_cols].to_numpy(_v611_np.float32)
assert _v611_np.isfinite(_final_vals).all(), "non-finite values in v611 final blend"
assert _final_vals.min() >= 0.0 and _final_vals.max() <= 1.0, "v611 final values outside [0,1]"
_nonconstant_cols = int((_final_df[_anchor_cols].max(axis=0) - _final_df[_anchor_cols].min(axis=0) > 1e-8).sum())
assert _nonconstant_cols > 0, "v611 final blend is all-constant"
_final_df.to_csv(FINAL_CSV, index=False)
print(
    f"v611 wrote {FINAL_CSV}: shape={_final_df.shape}, "
