...
REAL_SED_TIME_BUFFER_SEC = 55 * 60
REAL_SED_EST_SEC_PER_FILE_PER_MODEL = 6.0
REAL_SED_ZIP_NAME = 'sed-nfnet-v29-20s128-broad181-v1.zip'
REAL_SED_DATASET_SLUG = 'bc26-sed-nfnet-v29-20s128-broad181-v1'
...
def _sed_find_manifest():
    candidates = [
        ...
        Path('/kaggle/input') / REAL_SED_DATASET_SLUG / 'sed-nfnet-v29-20s128-broad181-v1' / 'sed_bundle_manifest.json',
        Path('/kaggle/working/sed_nfnet_v29_bundle/sed_bundle_manifest.json'),
    ]
    for c in candidates:
        if c.exists():
            return c
    ...
    for z in zip_candidates:
        if z.exists():
            out_dir = Path('/kaggle/working/sed_nfnet_v29_bundle')
...
def _sed_select_entries(entries, n_files):
    # Estimate remaining CPU budget aggressively. v526 timed out on hidden code
    # submission with three NFNet models, so v531 is a one-model fail-safe probe:
    # run v29 only when there is a large wall-clock buffer, otherwise fall back
    # to the already-safe v517 axis instead of burning the submission on timeout.
...
print(f'  Taxon max gate: floor={TAXON_MAX_GATE_FLOOR}, alpha={TAXON_MAX_GATE_ALPHA}')
print(f'  Post: v508 immediate temporal/logit local-event axis + fast single-model v29 20s/128 broad NFNet blend weight {REAL_SED_BLEND_WEIGHT}, then v517 taxon max gate')
