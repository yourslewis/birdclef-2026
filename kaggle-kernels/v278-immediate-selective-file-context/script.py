#!/usr/bin/env python3
"""
BirdCLEF 2026 — v278 Immediate Temporal + Selective File Context Kernel
=============================================================================
...
- Target behavior: improve over the current 0.923 LB anchor by combining v111/v118
  strengths plus immediate-neighbor temporal smoothing and selective top-k file-context boosting.
"""
...
print('V278: immediate-only temporal smoothing + selective top-k file-context boost')

# === Cell: Settings ===
# All high-impact knobs are centralized here so experiments can be audited.
# v278 keeps the known-strong v111/v118 family of settings, especially:
...
def file_context_boost(probs, alpha=FILE_CONTEXT_ALPHA, topk=FILE_CONTEXT_TOPK):
    ...
    if n_complete_files > 0:
        ...
        if topk is not None and 0 < topk < probs.shape[1]:
            ...
            # species per soundscape. This keeps the temporal-consistency benefit
            # while avoiding broad low-probability lifts across hundreds of classes.
