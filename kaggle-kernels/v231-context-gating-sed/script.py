...
class ContextGating(nn.Module):
    def __init__(self, in_features):
        super().__init__()
        self.cg = nn.Linear(in_features, in_features)
        
    def forward(self, x):
        # x shape: (Batch, Time, Features)
        # Learn an element-wise gating mask based on the feature vectors
        gate = torch.sigmoid(self.cg(x))
        return x * gate

class CGSEDHead(nn.Module):
    def __init__(self, in_features, num_classes):
        super().__init__()
        self.cg = ContextGating(in_features)
        self.att_fc = nn.Linear(in_features, num_classes)
        self.cla_fc = nn.Linear(in_features, num_classes)
        
    def forward(self, x):
        # x is (Batch, Channels, Time) -> reshape to (Batch, Time, Channels)
        if x.dim() == 4:
