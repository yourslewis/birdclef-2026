#!/usr/bin/env python3
"""
BirdCLEF 2026 — v318 v257 Smoothing + Tail Dampening Kernel
=============================================================================
...
- Target behavior: improve over the current 0.923 LB anchor by combining v111/v118
  strengths plus immediate-only temporal smoothing and stronger row-wise low-rank tail dampening.
"""
...
print('V318: immediate-only temporal smoothing + stronger low-rank tail dampening K5 p1.10')

# === Cell: Settings ===
# All high-impact knobs are centralized here so experiments can be audited.
# v245 keeps the known-strong v111/v118 family of settings, especially:
...
# v258 post-processing: keep only the strongest labels in each row unchanged
# and damp the low-rank tail more aggressively. Inspired by BirdCLEF 2025
# power-adjustment summaries that describe lowering weaker predictions.
TAIL_KEEP_TOP_K = 5
...
# v318: row-wise low-rank tail dampening. Keep top-K class probabilities per
# window unchanged; apply a mild suppressing power to the remaining labels.
...
print(f'  Post: immediate temporal smooth → sigmoid → file-context boost → gamma 0.85 → strong tail dampening')
