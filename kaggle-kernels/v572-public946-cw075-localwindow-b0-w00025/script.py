#!/usr/bin/env python3
"""
BirdCLEF 2026 v572 Public946 + cw0.75 local-window B0 sidecar.

Repo-owned controlled public946 stack plus a tiny rank sidecar from the cw0.75 local-window EfficientNet-B0 student. Exploratory candidate; base public946 logic remains unchanged.
"""
...
# %% cell 10
import os, re, gc, time, warnings, json, zipfile
...
# %% markdown
# ## v572 cw0.75 local-window B0 public946 student sidecar
#
# Source-clean v552 addition: load a private TorchScript EfficientNetV2-S student
# trained from the public946 sed85/rankblend15 blended teacher using a center/localmax radius-1 cw0.75 target and write `submission_cw075_localwindow_b0_student.csv`.
# The final blend applies this side stream as a conservative per-class rank sidecar.

# %% cell v572_cw075_localwindow_b0_sidecar
print("Starting v572 cw0.75 local-window B0 public946 student sidecar...")
STUDENT_CSV = "submission_cw075_localwindow_b0_student.csv"
STUDENT_RANK_BLEND = 0.0025
APPLY_TAXON_MAX_GATE = False
TAXON_MAX_GATE_FLOOR = 0.30
TAXON_MAX_GATE_ALPHA = 0.50
STUDENT_DATASET_SLUG = "bc26-public946-cw075-localwindow-b0-v1"
STUDENT_ZIP_NAME = "bc26-public946-cw075-localwindow-b0-v1.zip"

def _student_hz_to_mel(hz):
    return 2595.0 * np.log10(1.0 + np.asarray(hz) / 700.0)

def _student_mel_to_hz(mel):
    return 700.0 * (10.0 ** (np.asarray(mel) / 2595.0) - 1.0)

def _student_make_mel_filter(sr, n_fft, n_mels, fmin=20.0, fmax=None):
    if fmax is None:
        fmax = sr / 2
    mel_points = np.linspace(_student_hz_to_mel(fmin), _student_hz_to_mel(fmax), n_mels + 2)
    hz_points = _student_mel_to_hz(mel_points)
...
def _student_find_manifest():
    candidates = [
        Path("/kaggle/input") / STUDENT_DATASET_SLUG / "sed_bundle_manifest.json",
        Path("/kaggle/input/datasets/yourslewis") / STUDENT_DATASET_SLUG / "sed_bundle_manifest.json",
        Path("/kaggle/working/cw075_localwindow_b0_bundle/sed_bundle_manifest.json"),
    ]
    for c in candidates:
        if c.exists():
            return c
    hits = sorted(Path("/kaggle/input").glob("**/sed_bundle_manifest.json"))
    if hits:
        print("Student manifest candidates:", [str(x) for x in hits[:5]])
        return hits[0]
    zips = [Path("/kaggle/input") / STUDENT_DATASET_SLUG / STUDENT_ZIP_NAME, Path("/kaggle/input") / STUDENT_ZIP_NAME]
    zips += list(Path("/kaggle/input").glob(f"**/{STUDENT_ZIP_NAME}"))
    for z in zips:
        if z.exists():
            out = Path("/kaggle/working/cw075_localwindow_b0_bundle")
            out.mkdir(parents=True, exist_ok=True)
            manifest = out / "sed_bundle_manifest.json"
            if not manifest.exists():
                print(f"Extracting cw0.75 local-window B0 student bundle from {z} ...")
                with zipfile.ZipFile(z, "r") as zf:
                    zf.extractall(out)
            if manifest.exists():
                return manifest
    try:
        print("Kaggle input roots:", [p.name for p in Path("/kaggle/input").iterdir()])
    except Exception as exc:
        print(f"WARNING: could not list /kaggle/input: {exc}")
    raise FileNotFoundError("cw0.75 local-window B0 student bundle manifest not found")

def _student_decode_audio(path, sr, target_samples):
    try:
        ...
        print(f"WARNING: ffmpeg student decode failed for {path.name}: {exc}; trying soundfile fallback")
...
def _student_extract_context(audio, sr, end_sec, context_sec):
...
def _student_waveform_to_logmel(wave, audio_cfg, mel_fb):
...
def _student_infer_bundle(paths):
    manifest_path = _student_find_manifest()
    print(f"cw0.75 local-window B0 student manifest: {manifest_path}")
    manifest = json.loads(manifest_path.read_text())
    labels = [str(x) for x in manifest["labels"]]
    audio_cfg = manifest["audio_config"]
    entries = manifest.get("models", [])
    if not entries:
        raise RuntimeError("cw0.75 local-window B0 student manifest has no models")
    ...
    weight_sum = sum(float(e.get("weight", 0.0)) for e, _ in models) or float(len(models))
    sr = int(audio_cfg["sample_rate"])
    context_sec = float(audio_cfg["duration_sec"])
    mel_fb = _student_make_mel_filter(sr, int(audio_cfg["n_fft"]), int(audio_cfg["n_mels"]))
    rows, probs_all = [], []
    t0 = time.time()
    for i, path in enumerate(paths, 1):
        audio = _student_decode_audio(path, sr, int(round(sr * 60.0)))
        windows = [
            _student_waveform_to_logmel(torch.from_numpy(_student_extract_context(audio, sr, end_sec, context_sec).copy()), audio_cfg, mel_fb)
            for end_sec in range(5, 65, 5)
        ]
        x_all = torch.stack(windows)
        file_probs = np.zeros((N_WINDOWS, len(labels)), dtype=np.float32)
        with torch.no_grad():
            for entry, model in models:
                chunks = []
                for start in range(0, len(x_all), 4):
                    out = model(x_all[start:start + 4])
                    clip_logits = out[0] if isinstance(out, (tuple, list)) else out
                    chunks.append(torch.sigmoid(clip_logits).cpu().numpy().astype(np.float32))
                file_probs += (float(entry.get("weight", 0.0)) / weight_sum) * np.concatenate(chunks, axis=0)
        rows.extend([f"{path.stem}_{end_sec}" for end_sec in range(5, 65, 5)])
        probs_all.append(file_probs)
        if i == 1 or i % 10 == 0 or i == len(paths):
            print(f"cw0.75 local-window B0 student inferred {i}/{len(paths)} files; elapsed={time.time() - t0:.1f}s")
    arr = np.vstack(probs_all)
    pred = pd.DataFrame(arr, columns=labels)
    pred.insert(0, "row_id", rows)
    for col in PRIMARY_LABELS:
        if col not in pred.columns:
            pred[col] = 0.0
    pred = pred[["row_id", *PRIMARY_LABELS]]
    vals = np.clip(pred[PRIMARY_LABELS].to_numpy(np.float32), 0.0, 1.0)
    pred[PRIMARY_LABELS] = vals
    print(f"cw0.75 local-window B0 student prob range: {vals.min():.6f} to {vals.max():.6f}, mean={vals.mean():.6f}; runtime={time.time() - t0:.1f}s")
    return pred

_student_sub = _student_infer_bundle(test_paths)
_student_sub.to_csv(STUDENT_CSV, index=False)
print(f"cw0.75 local-window B0 student sidecar complete: {STUDENT_CSV} {_student_sub.shape}, weight={STUDENT_RANK_BLEND}")
...
df_student = df_student.set_index("row_id").loc[df_proto["row_id"]].reset_index()
p_proto = np.clip(df_proto[cols].to_numpy(np.float32), EPS, 1.0 - EPS)
p_sed   = np.clip(df_sed[cols].to_numpy(np.float32), EPS, 1.0 - EPS)
p_student = np.clip(df_student[cols].to_numpy(np.float32), EPS, 1.0 - EPS)
...
rank_student = pd.DataFrame(p_student).rank(axis=0, pct=True).to_numpy(np.float32)

print("Executing standard public946 2-way rank blend (60% Proto / 40% SED)...")
...
pred = (1.0 - STUDENT_RANK_BLEND) * pred + STUDENT_RANK_BLEND * rank_student
pred = np.clip(pred, 0.0, 1.0).astype(np.float32)
print(f"Applied cw0.75 local-window B0 student rank sidecar blend: weight={STUDENT_RANK_BLEND}")
...
try:
    ...
    if APPLY_TAXON_MAX_GATE:
        taxon_values = sub[cols].to_numpy(np.float32).copy()
        taxon_group_count = 0
        for class_name in sorted(set(str(x) for x in tax_df["class_name"].dropna().tolist())):
            idx = [i for i, label in enumerate(cols) if label in tax_df.index and str(tax_df.loc[label, "class_name"]) == class_name]
            if not idx:
                continue
            evidence = taxon_values[:, idx].max(axis=1, keepdims=True)
            mult = np.maximum(float(TAXON_MAX_GATE_FLOOR), evidence) ** float(TAXON_MAX_GATE_ALPHA)
            taxon_values[:, idx] *= mult
            taxon_group_count += 1
        sub[cols] = np.clip(taxon_values, 0.0, 1.0).astype(np.float32)
        print(f"Taxon max gate applied to {taxon_group_count} class groups: floor={TAXON_MAX_GATE_FLOOR}, alpha={TAXON_MAX_GATE_ALPHA}")
    else:
        print("Taxon max gate disabled.")
except Exception as e:
    print(f"Adaptive thresholding / taxon max gate skipped: {e}")
