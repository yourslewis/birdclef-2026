...
def apply_cooccurrence_filter(preds, cooc_matrix, alpha=0.1):
    # preds: (N_samples, N_classes)
    # cooc_matrix: (N_classes, N_classes) pre-computed conditional probabilities
    # P(Class A | Class B) from the training set metadata
    
    adjusted_preds = np.copy(preds)
    for i in range(len(preds)):
        # For each sample, if a high-confidence bird exists, it slightly boosts its likely companions
        # and suppresses its unlikely companions
        base_probs = preds[i]
        
        # Calculate context vector
        context = np.dot(base_probs, cooc_matrix)
        # Normalize context
        if np.max(context) > 0:
            context = context / np.max(context)
            
        # Blend original predictions with context
        adjusted_preds[i] = base_probs * (1 - alpha) + context * alpha
