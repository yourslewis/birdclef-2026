...
def rank_average(predictions_list, weights=None):
    # predictions_list: list of 2D numpy arrays (N_samples, N_classes)
    if weights is None:
        weights = [1.0] * len(predictions_list)
        
    weights = np.array(weights) / np.sum(weights)
    avg_ranks = np.zeros_like(predictions_list[0])
    
    for w, preds in zip(weights, predictions_list):
        # Rank within each sample across classes. Higher probability = higher rank.
        # rankdata returns 1 for lowest, N_classes for highest.
        # We normalize to [0, 1] so it acts like a pseudo-probability for thresholding.
        ranks = np.zeros_like(preds)
        for i in range(len(preds)):
            ranks[i] = rankdata(preds[i], method='min') / preds.shape[1]
