#!/usr/bin/env python3
"""
BirdCLEF 2026 v541 public946 mirror/rare replay.

Repo-owned controlled port of the public [LB] [SCORE] [0.946] stack
(safar1/lb-score-0-946).  Compared with v539, this version keeps the
public sonotype mirroring and adaptive rare-taxon suppression postprocess.
"""
...
def _build_cache():
    ...
    print(
        f"  Perch pass done in {time.time() - t0:.1f}s  "
...
print(
    f"sc_tr: {sc_tr.shape}  emb_tr: {emb_tr.shape}  "
...
def apply_mlp_probes_vectorized(emb_test, scores_test, probe_models, scaler, pca, alpha_blend=0.4):
    ...
    result[:, valid_classes] = (
        (1.0 - alpha_blend) * scores_test[:, valid_classes] + alpha_blend * preds.T
    )
    return result
