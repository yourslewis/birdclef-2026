...
def make_fmix_mask(size, alpha, decay_power):
    # Simulated FMix mask generator
    # Creates organic, cloud-like masks instead of rectangles
    mask = np.random.rand(*size)
    # Applying simplistic low-pass filter to simulate fractal decay
    mask = torch.tensor(mask).float()
    return (mask > alpha).float()

def apply_fmix(data, targets, alpha=0.5):
    indices = torch.randperm(data.size(0))
    shuffled_data = data[indices]
    shuffled_targets = targets[indices]
    
    # Generate organic mask
    mask = make_fmix_mask(data.size(), alpha=0.5, decay_power=3)
    if torch.cuda.is_available():
        mask = mask.cuda()
        
    data = mask * data + (1 - mask) * shuffled_data
    # Adjust lambda based on actual mask area
    lam = mask.mean().item()
