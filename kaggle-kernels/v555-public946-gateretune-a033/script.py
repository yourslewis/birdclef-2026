#!/usr/bin/env python3
"""
BirdCLEF 2026 v555 conservative public946 gate-retune interpolation candidate.
...
print("Executing v555 conservative gate-retune rank blend (v542 baseline + 33% v554 retune)...")
...
# v555 risk-control: interpolate fully postprocessed v542 baseline with the
# locally stronger v554 retune. Offline gate: alpha=0.33 kept most of the
# dry-run AUC lift while reducing displacement to corr~0.99937 / MAE~0.0042.
V555_FULL_RETUNE_ALPHA = 0.33
...
try:
    ...
    print(f"v555 baseline reconstruction rare thresholding skipped: {e}")

sub_values = sub[cols].to_numpy(np.float32)
base_values = baseline_sub[cols].to_numpy(np.float32)
sub[cols] = ((1.0 - V555_FULL_RETUNE_ALPHA) * base_values + V555_FULL_RETUNE_ALPHA * sub_values).astype(np.float32)
print(f"v555 conservative interpolation alpha={V555_FULL_RETUNE_ALPHA}")
...
print(f"v555 gate retune params: proto_weight={V554_PROTO_WEIGHT}, fake_boost={V554_FAKE_BOOST}, ctx_thr={V554_CTX_THR}, ctx_boost={V554_CTX_BOOST}, sed_rank_thr={V554_SED_RANK_THR}, sed_boost={V554_SED_BOOST}, rare_scale={V554_RARE_SCALE}, alpha={V555_FULL_RETUNE_ALPHA}")
print("v555 conservative blend and post-processing complete. Ready for gated dry-run review!")
