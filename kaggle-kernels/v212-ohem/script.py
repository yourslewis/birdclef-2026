...
# Online Hard Example Mining (OHEM) wrapper around BCE
class OHEMBCEWithLogitsLoss(nn.Module):
    def __init__(self, top_k_ratio=0.7):
        super().__init__()
    ...
    def forward(self, logits, targets):
        ...
        sample_losses = loss.mean(dim=1) 
        
        # Determine how many samples to keep
        k = int(self.top_k_ratio * logits.size(0))
        if k == 0:
            return loss.mean()
            
        # Pick the top k hardest examples
