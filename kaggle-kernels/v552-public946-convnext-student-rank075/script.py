...
STUDENT_ZIP_NAME = "bc26-public946-convnext-tiny-student-v1.zip"
...
def _student_find_manifest():
    candidates = [
        ...
        Path("/kaggle/working/convnext_student_bundle/sed_bundle_manifest.json"),
    ]
    for c in candidates:
        if c.exists():
            return c
    ...
    for z in zips:
        if z.exists():
            out = Path("/kaggle/working/convnext_student_bundle")
            out.mkdir(parents=True, exist_ok=True)
            manifest = out / "sed_bundle_manifest.json"
            if not manifest.exists():
                print(f"Extracting ConvNeXt student bundle from {z} ...")
    ...
    raise FileNotFoundError("ConvNeXt student bundle manifest not found")
...
def _student_infer_bundle(paths):
    ...
    print(f"ConvNeXt student manifest: {manifest_path}")
    manifest = json.loads(manifest_path.read_text())
    labels = [str(x) for x in manifest["labels"]]
    audio_cfg = manifest["audio_config"]
    entries = manifest.get("models", [])
    if not entries:
        raise RuntimeError("ConvNeXt student manifest has no models")
    ...
    for i, path in enumerate(paths, 1):
        ...
        if i == 1 or i % 10 == 0 or i == len(paths):
            print(f"ConvNeXt student inferred {i}/{len(paths)} files; elapsed={time.time() - t0:.1f}s")
    ...
    print(f"ConvNeXt student prob range: {vals.min():.6f} to {vals.max():.6f}, mean={vals.mean():.6f}; runtime={time.time() - t0:.1f}s")
    return pred

_student_sub = _student_infer_bundle(test_paths)
_student_sub.to_csv(STUDENT_CSV, index=False)
print(f"ConvNeXt student sidecar complete: {STUDENT_CSV} {_student_sub.shape}, weight={STUDENT_RANK_BLEND}")
...
print(f"Applied ConvNeXt student rank sidecar blend: weight={STUDENT_RANK_BLEND}")
