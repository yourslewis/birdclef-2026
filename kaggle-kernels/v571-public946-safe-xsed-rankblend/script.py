...
# v571: repo-owned safe xSED-inspired final recipe.  This keeps v542's
# input discovery, ProtoSSM branch, SED branch, and post-blend gates intact;
# only the initial Proto/SED rank weighting is nudged to the public-source
# xSED ratio observed in pilkwang's safe-ensemble logs.
V571_PROTO_RANK_WEIGHT = 0.5964
V571_SED_RANK_WEIGHT = 1.0 - V571_PROTO_RANK_WEIGHT
...
print(
    "Executing v571 safe xSED-inspired 2-way rank blend "
    f"({V571_PROTO_RANK_WEIGHT:.4f} Proto / {V571_SED_RANK_WEIGHT:.4f} SED)..."
)
pred = (rank_proto * V571_PROTO_RANK_WEIGHT) + (rank_sed * V571_SED_RANK_WEIGHT)
...
def verify_v571_submission(frame: pd.DataFrame) -> None:
    """Hard-fail on schema/shape fallbacks before writing submission.csv."""
    sample_cols = pd.read_csv(BASE / "sample_submission.csv").columns.tolist()
    assert frame.columns.tolist() == sample_cols, "submission columns do not match sample_submission.csv"
    assert frame["row_id"].is_unique, "duplicate row_id values in submission"
    assert len(frame) > 0, "empty submission"

    values = frame[cols].to_numpy(np.float32)
    assert np.isfinite(values).all(), "non-finite values in submission"
    assert values.min() >= 0.0 and values.max() <= 1.0, "submission values outside [0, 1]"

    if IS_DRY_RUN:
        expected_rows = len(df_proto)
        assert len(frame) == expected_rows, f"dry-run row count mismatch: {len(frame)} != {expected_rows}"
    else:
        expected_rows = len(test_paths) * 12
        assert len(frame) == expected_rows, f"hidden/test row count mismatch: {len(frame)} != {expected_rows}"
        assert len(frame) > 3, "formal submission collapsed to sample-output shape"

    print(
        "v571 verifier passed: "
        f"rows={len(frame)}, cols={len(frame.columns)}, dry_run={IS_DRY_RUN}, "
        f"min={values.min():.6f}, max={values.max():.6f}"
    )

verify_v571_submission(sub)
sub.to_csv(OUT_CSV, index=False)
print("v571 safe xSED rank blend and post-processing complete. Ready for submission!")
