#!/usr/bin/env python3
"""
BirdCLEF 2026 - v509 No-Call Confidence Gate Kernel
=============================================================================
...
print('V509: SED local logit-event top3 + no-call confidence gate + EW0.625 + gamma 0.825 + context 0.275')

# === Cell: Settings ===
# All high-impact knobs are centralized here so experiments can be audited.
# v509 keeps the known-strong v111/v118 family of settings, especially:
...
# v509: SED-inspired local event propagation in logit space. v501-v509
# tied best at context0.25 while v500 context0.20 slipped; v509 keeps v509
...
# v509 Spec E: conservative no-call/background confidence gate. 2025-style
# systems used separate background/non-bird handling; this executable variant
# attenuates rows whose best class remains weak after all ensemble/postprocess
# signals, while leaving confident event rows essentially unchanged.
NO_CALL_GATE_ALPHA = 0.12
NO_CALL_GATE_THRESHOLD = 0.08
NO_CALL_GATE_TEMPERATURE = 0.02
NO_CALL_GATE_FLOOR = 0.88
...
def no_call_confidence_gate(
    probs,
    alpha=NO_CALL_GATE_ALPHA,
    threshold=NO_CALL_GATE_THRESHOLD,
    temperature=NO_CALL_GATE_TEMPERATURE,
    floor=NO_CALL_GATE_FLOOR,
):
    if alpha <= 0:
        return probs, np.ones((probs.shape[0], 1), dtype=probs.dtype)
    row_conf = probs.max(axis=1, keepdims=True)
    temp = max(float(temperature), 1e-6)
    low_signal = 1.0 / (1.0 + np.exp(-np.clip((threshold - row_conf) / temp, -30, 30)))
    gate = 1.0 - alpha * low_signal
    gate = np.maximum(gate, floor).astype(probs.dtype)
    return probs * gate, gate
...
probs, no_call_gate = no_call_confidence_gate(probs)
probs = np.clip(probs, 1e-8, 1.0 - 1e-8)
print(
    f'No-call gate range: {no_call_gate.min():.4f} to {no_call_gate.max():.4f}, '
    f'mean: {no_call_gate.mean():.4f}'
)
...
print(f'  Post: immediate temporal smooth, top3 logit local-event boost, sigmoid, file-context alpha 0.275, gamma 0.825, top-k contrast, no-call confidence gate, ProtoSSM EW0.625')
