...
# | CLAP INT8 side stream | Habedi CLAP INT8 bundle, source-cleaned from `xiyuetong/birdclef2026-ensemble-v2` |
...
# %% markdown
# ## CLAP INT8 ONNX side stream
#
# Source-clean v545 addition: attach the public `habedi/birdclef-2026-clap-int8-bundle`
# and run its ONNX CLAP audio encoder + 5-fold probe as a small orthogonal stream.
# This is intentionally conservative: it writes `submission_clap_onnx.csv`, then the
# final blend uses only 0.5% CLAP rank weight.  Missing/failed CLAP is a hard failure
# for this candidate so we do not silently submit the plain public946 fallback.

# %% cell v551_clap_int8
print("Starting v551 CLAP INT8 0.5% ONNX side stream...")
...
if not _ONNX_AVAILABLE:
    raise RuntimeError("onnxruntime is not available; cannot run v551 CLAP INT8 0.5% stream")

try:
    _so = ort.SessionOptions()
    _so.intra_op_num_threads = 2
    _csess = ort.InferenceSession(str(_clap_onnx_model), sess_options=_so, providers=["CPUExecutionProvider"])

    _pw = np.load(_CLAP_ONNX_DIR / "probe_weights.npz")
    _ln_w = [_pw[f"f{f}_net.0.weight"] for f in range(5)]
    _ln_b = [_pw[f"f{f}_net.0.bias"] for f in range(5)]
    _w1 = [_pw[f"f{f}_net.1.weight"] for f in range(5)]
    _b1 = [_pw[f"f{f}_net.1.bias"] for f in range(5)]
    _w2 = [_pw[f"f{f}_net.4.weight"] for f in range(5)]
    _b2 = [_pw[f"f{f}_net.4.bias"] for f in range(5)]
    _n_cls = int(_w2[0].shape[0])
    if _n_cls != N_CLASSES:
        raise ValueError(f"CLAP probe class mismatch: {_n_cls} vs expected {N_CLASSES}")

    def _v545_cprobe(emb):
        logits = np.zeros(_n_cls, dtype=np.float32)
        for f in range(5):
            h = (emb - emb.mean()) / (emb.std() + 1e-8)
            h = h * _ln_w[f] + _ln_b[f]
            h = h @ _w1[f].T + _b1[f]
            h = np.maximum(h, 0)
            h = h @ _w2[f].T + _b2[f]
            logits += h
        return 1.0 / (1.0 + np.exp(-logits / 5.0))
    ...
    _HOP = 512
    _TF = 1001
    _WS = int(WINDOW_SEC * _CLAP_SR)
    _mel_f = np.load(_CLAP_ONNX_DIR / "mel_filters_slaney.npy")  # (513, 64)
    _hann = np.hanning(_N_FFT + 1)[:_N_FFT]

    def _v545_fast_mel(audio):
        _, _, spec = _sig.stft(
        ...
        mel = (np.abs(spec) ** 2).T @ _mel_f
        mel = np.clip(mel, 1e-10, None)
        mel_db = 10.0 * np.log10(mel)
        T = mel_db.shape[0]
        if T < _TF:
            mel_db = np.pad(mel_db, ((0, _TF - T), (0, 0)), mode="constant")
        elif T > _TF:
            start = (T - _TF) // 2
            mel_db = mel_db[start:start + _TF]
        mu, sg = mel_db.mean(), mel_db.std() + 1e-8
        return ((mel_db - mu) / sg)[np.newaxis, np.newaxis, ...].astype(np.float32)

    _n_test = len(test_paths)
    _cprobs = np.zeros((_n_test * N_WINDOWS, N_CLASSES), dtype=np.float32)
    _rids = []
    _t0 = time.time()
    _BUDGET = 45 * 60
    ...
    for fi, fp in enumerate(test_paths):
        if time.time() - _t0 > _BUDGET:
            raise TimeoutError(f"CLAP INT8 budget exhausted at file {fi}/{_n_test}")
        ...
        for wi in range(N_WINDOWS):
            ...
            w = y[s:e]
            if len(w) < _WS:
                w = np.pad(w, (0, _WS - len(w)))
            mel = _v545_fast_mel(w)
            emb = _csess.run(["pooler"], {"input_features": mel})[0]
            _cprobs[fi * N_WINDOWS + wi] = _v545_cprobe(emb[0])
            _rids.append(f"{stem}_{(wi + 1) * WINDOW_SEC}")
        print(f"CLAP INT8: {fi + 1}/{_n_test} files, elapsed={time.time() - _t0:.1f}s")

    _clap_sub = pd.DataFrame({"row_id": _rids})
    for j, c in enumerate(PRIMARY_LABELS):
        _clap_sub[c] = _cprobs[:, j]
    _clap_sub.to_csv("submission_clap_onnx.csv", index=False)
    _clap_ok = True
    print(
        f"CLAP INT8 side stream complete: submission_clap_onnx.csv {_clap_sub.shape}, "
        f"prob_range=({_cprobs.min():.6f},{_cprobs.max():.6f}), mean={_cprobs.mean():.6f}, "
        f"runtime={time.time() - _t0:.1f}s"
    )
finally:
    try:
        del _csess
...
print(f"CLAP INT8 available: {_clap_ok}")


# %% markdown
# ## Rank Blend & Fat-Tail Continuity Gates
#
# This final step computes the standard public946 Proto/SED rank ensemble and then applies a tiny 0.5% CLAP INT8 rank sidecar after the public946 continuity gates.
#
...
CLAP_CSV     = "submission_clap_onnx.csv"
OUT_CSV      = "submission.csv"
CLAP_RANK_BLEND = 0.005
...
df_clap = df_clap.set_index("row_id").loc[df_proto["row_id"]].reset_index()
p_proto = np.clip(df_proto[cols].to_numpy(np.float32), EPS, 1.0 - EPS)
p_sed   = np.clip(df_sed[cols].to_numpy(np.float32), EPS, 1.0 - EPS)
p_clap  = np.clip(df_clap[cols].to_numpy(np.float32), EPS, 1.0 - EPS)

if not np.isfinite(p_clap).all():
    raise ValueError("CLAP predictions contain NaN/Inf")
if float(p_clap.max()) <= EPS:
    raise ValueError("CLAP predictions are all zero; refusing silent fallback")
...
rank_clap  = pd.DataFrame(p_clap).rank(axis=0, pct=True).to_numpy(np.float32)

print("Executing v551 public946 2-way rank blend plus 0.5% CLAP INT8 sidecar...")
...
pred = (1.0 - CLAP_RANK_BLEND) * pred + CLAP_RANK_BLEND * rank_clap
pred = np.clip(pred, 0.0, 1.0).astype(np.float32)
print(f"Applied CLAP INT8 rank sidecar blend: weight={CLAP_RANK_BLEND}")
