#!/usr/bin/env python3
"""
BirdCLEF 2026 v558 exact-base clipped public946 gate-retune candidate.
...
# v558 exact-base low-displacement retune. Start from the exact standard v542
# final `sub`, then add a clipped low-alpha retuned-gate delta. This avoids the
# baseline reconstruction mismatch seen in v557.
V558_PROTO_WEIGHT = 0.56
V558_FAKE_BOOST = 0.04
V558_CTX_THR = 0.90
V558_CTX_BOOST = 0.10
V558_SED_RANK_THR = 0.93
V558_SED_BOOST = 0.08
V558_RARE_SCALE = 0.85
V558_FULL_RETUNE_ALPHA = 0.10
V558_MAX_ABS_DELTA = 0.02

baseline_values = sub[cols].to_numpy(np.float32).copy()
retune_pred = (rank_proto * V558_PROTO_WEIGHT) + (rank_sed * (1.0 - V558_PROTO_WEIGHT))
retune_fake_only = (p_proto > 0.50) & (p_sed < 0.05)
retune_pred = np.where(retune_fake_only, (1.0 - V558_FAKE_BOOST) * retune_pred + V558_FAKE_BOOST * rank_proto, retune_pred)
retune_proto_cont = (xctx > V558_CTX_THR) & (rank_proto > 0.75) & (p_sed < 0.12) & (~retune_fake_only)
retune_pred = np.where(retune_proto_cont, (1.0 - V558_CTX_BOOST) * retune_pred + V558_CTX_BOOST * np.maximum(rank_proto, xctx), retune_pred)
retune_sed_only = (rank_sed > V558_SED_RANK_THR) & (rank_proto < 0.80) & (~retune_fake_only) & (~retune_proto_cont)
retune_pred = np.where(retune_sed_only, (1.0 - V558_SED_BOOST) * retune_pred + V558_SED_BOOST * rank_sed, retune_pred)

retune_sub = df_proto.copy()
retune_sub[cols] = retune_pred.astype(np.float32)
for group in MIRROR_PAIRS:
    valid_idx = [col_to_idx[s] for s in group if s in col_to_idx]
    if len(valid_idx) >= 2:
        group_max = retune_sub[cols].iloc[:, valid_idx].max(axis=1).to_numpy(np.float32)
        for idx in valid_idx:
            retune_sub.iloc[:, idx + 1] = group_max
try:
    tax_df_v558 = pd.read_csv(BASE / "taxonomy.csv").set_index("primary_label")
    rare_classes_v558 = {"Amphibia", "Mammalia", "Reptilia"}
    for ci, species in enumerate(cols):
        if species in tax_df_v558.index and tax_df_v558.loc[species, "class_name"] in rare_classes_v558:
            col_idx = ci + 1
            vals = retune_sub.iloc[:, col_idx].to_numpy(np.float32)
            thr = vals.mean() + 0.05
            retune_sub.iloc[:, col_idx] = np.where(vals < thr, vals * V558_RARE_SCALE, vals)
except Exception as e:
    print(f"v558 retune rare thresholding skipped: {e}")

retune_values = retune_sub[cols].to_numpy(np.float32)
retune_delta = V558_FULL_RETUNE_ALPHA * (retune_values - baseline_values)
retune_delta = np.clip(retune_delta, -V558_MAX_ABS_DELTA, V558_MAX_ABS_DELTA)
sub[cols] = (baseline_values + retune_delta).astype(np.float32)
print(f"v558 exact-base clipped retune: alpha={V558_FULL_RETUNE_ALPHA}, max_abs_delta={V558_MAX_ABS_DELTA}, proto_weight={V558_PROTO_WEIGHT}, fake_boost={V558_FAKE_BOOST}, ctx_thr={V558_CTX_THR}, ctx_boost={V558_CTX_BOOST}, sed_rank_thr={V558_SED_RANK_THR}, sed_boost={V558_SED_BOOST}, rare_scale={V558_RARE_SCALE}")
...
print("v558 exact-base clipped blend and post-processing complete. Ready for gated dry-run review!")
