...
class TransformerSEDHead(nn.Module):
    def __init__(self, in_features, num_classes, num_heads=4, hidden_dim=512):
        super().__init__()
        self.in_features = in_features
        # Reduce dimensionality to keep Transformer computationally cheap
        self.proj = nn.Linear(in_features, hidden_dim)
        
        # 1-layer Transformer Encoder to share temporal context
        ...
        # Standard SED Attention pooling
        self.att_fc = nn.Linear(hidden_dim, num_classes)
        self.cla_fc = nn.Linear(hidden_dim, num_classes)
        
    def forward(self, x):
        # x is expected to be (Batch, Channels, Time) -> reshape to (Batch, Time, Channels)
        if x.dim() == 4:
            x = x.mean(dim=2)  # collapse frequency dimension if it exists
        x = x.transpose(1, 2)
        
        x = self.proj(x)
        # Apply self-attention across the time dimension
        x = self.transformer(x)
        
        # Frame-level predictions
        framewise_preds = torch.sigmoid(self.cla_fc(x))
        # Frame-level attention weights
        attention = torch.softmax(self.att_fc(x), dim=1)
        
        # Clip-level prediction (weighted sum of framewise preds)
        clipwise_preds = torch.sum(framewise_preds * attention, dim=1)
        
        return clipwise_preds, framewise_preds
