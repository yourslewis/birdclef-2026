...
def power_average(predictions, weight=3.0):
    # predictions: list of numpy arrays
    import numpy as np
    preds_stack = np.stack(predictions)
    return np.mean(preds_stack ** weight, axis=0) ** (1.0 / weight)

# Use geometric mean as backup if power average is too extreme for some classes
def geometric_average(predictions):
    import numpy as np
    preds_stack = np.stack(predictions)
    return np.exp(np.mean(np.log(np.clip(preds_stack, 1e-7, 1.0)), axis=0))
