#!/usr/bin/env python3
"""
BirdCLEF 2026 — v282 Immediate Temporal + Gentle File Mean Blend Kernel
=============================================================================
...
- Target behavior: improve over the current 0.923 LB anchor by combining v111/v118
  strengths plus immediate-neighbor temporal smoothing and gentle BirdCLEF-2024-style within-file mean blending.
"""
...
print('V282: immediate-only temporal smoothing + gentle file mean blend context alpha 0.10')

# === Cell: Settings ===
# All high-impact knobs are centralized here so experiments can be audited.
# v282 keeps the known-strong v111/v118 family of settings, especially:
...
def file_context_boost(probs, alpha=FILE_CONTEXT_ALPHA):
    ...
    if n_complete_files > 0:
        ...
        # with its per-file mean after temporal smoothing. v282 uses a gentler
        # alpha=0.10 variant: enough repeated-soundscape context to damp isolated
        # spikes, but less flattening than v281's alpha=0.20.
...
print(f'  Post: Gaussian smooth → sigmoid → gentle file-mean blend → power transform')
