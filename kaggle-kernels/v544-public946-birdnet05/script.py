...
# %% markdown
# ### BirdNET 6K external acoustic stream
#
# Source-clean 3-way candidate: run only the resolved BirdNET TFLite stream
# and blend it conservatively with the public946 Proto/SED anchor.  The
# custom EffNet stream from the public 4-way notebook is intentionally not
# imported because its notebook-output source is blocked from this account.

# %% cell 64
import librosa as _birdnet_librosa
import tensorflow as _birdnet_tf
import soundfile as _birdnet_sf

BIRDNET_MODEL_DIR = Path(
    "/kaggle/input/models/shadiakiki1/birdnet-analyzer/"
    "tflite/birdnet_global_6k_v2.4_model_fp32-1/3"
)
if not BIRDNET_MODEL_DIR.exists():
    raise FileNotFoundError(f"BirdNET model directory not found: {BIRDNET_MODEL_DIR}")

model_files = list(BIRDNET_MODEL_DIR.glob("*Model_FP32.tflite")) or list(BIRDNET_MODEL_DIR.glob("*.tflite"))
if not model_files:
    raise FileNotFoundError(f"BirdNET TFLite model not found under: {BIRDNET_MODEL_DIR}")
BIRDNET_MODEL_PATH = model_files[0]
print(f"Using BirdNET model: {BIRDNET_MODEL_PATH}")

label_files = list(BIRDNET_MODEL_PATH.parent.glob("*.txt"))
if not label_files:
    label_files = list(BIRDNET_MODEL_PATH.parent.parent.glob("**/*.txt"))
if not label_files:
    raise FileNotFoundError(f"BirdNET label file not found near: {BIRDNET_MODEL_PATH.parent}")

with open(label_files[0], "r") as f:
    bn_labels = [line.strip().split("_")[0] for line in f.readlines()]

tax_df_bn = pd.read_csv(BASE / "taxonomy.csv")
bn_idx_map = []
mapped_mask = []
for label in PRIMARY_LABELS:
    sci = tax_df_bn.loc[tax_df_bn["primary_label"] == label, "scientific_name"].values
    sci_name = sci[0] if len(sci) else ""
...
bn_idx_map = np.asarray(bn_idx_map, dtype=np.int64)
mapped_mask = np.asarray(mapped_mask, dtype=bool)
mapped_count = int(mapped_mask.sum())
print(f"BirdNET mapped classes: {mapped_count} / {len(PRIMARY_LABELS)}")
if mapped_count == 0:
    raise RuntimeError("BirdNET mapped zero competition labels; refusing silent 2-way fallback")

interpreter = _birdnet_tf.lite.Interpreter(model_path=str(BIRDNET_MODEL_PATH))
...
bn_preds = []
_bn_start = time.time()
for path in tqdm(test_paths, desc="BirdNET 6K"):
    y, sr0 = _birdnet_sf.read(str(path), dtype="float32", always_2d=False)
    if getattr(y, "ndim", 1) == 2:
        y = y.mean(axis=1)
    if sr0 != 48000:
        y = _birdnet_librosa.resample(y, orig_sr=sr0, target_sr=48000)
    ...
    file_preds = []
    for chunk in chunks_3s:
        interpreter.set_tensor(in_idx, chunk[None, :].astype(np.float32))
        interpreter.invoke()
        out = interpreter.get_tensor(out_idx)[0]
        p = np.zeros(N_CLASSES, dtype=np.float32)
        p[mapped_mask] = out[bn_idx_map[mapped_mask]]
        file_preds.append(p)

    bn_rows.extend([f"{path.stem}_{int(t)}" for t in ends])
    bn_preds.extend(file_preds)

bn_sub = pd.DataFrame(np.clip(np.asarray(bn_preds, dtype=np.float32), 0.0, 1.0), columns=PRIMARY_LABELS)
bn_sub.insert(0, "row_id", bn_rows)
bn_sub.to_csv("submission_birdnet.csv", index=False)
print(f"BirdNET 6K Inference Complete. Saved submission_birdnet.csv in {time.time() - _bn_start:.1f}s")
...
print("Executing 3-way rank blend (56% Proto / 39% SED / 5% BirdNET)...")
pred = (rank_proto * 0.56) + (rank_sed * 0.39) + (rank_bn * 0.05)
