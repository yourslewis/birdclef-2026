...
# %% markdown
# ## v547 CV9245 Sidecar Inference
#
# Source-clean v547 addition: run the public CV9245 MoE/CNN sidecar using the
# already-computed Perch logits/embeddings from the public946 base.  Missing
# artifacts or shape mismatches are hard failures; this candidate must not fall
# back to the plain public946 anchor silently.

# %% cell v547_cv9245_sidecar
print("Starting v547 CV9245 sidecar from shared public946 Perch outputs...")
import importlib.util as _cv_importlib_util

CV9245_RANK_BLEND = 0.005
CV9245_PRIOR_SCALE = 0.45
CV9245_CNN_WEIGHT = 0.20

_cv_script_hits = sorted(Path("/kaggle/input").rglob("pantanal_infer_only_submission.py"))
print("CV9245 script candidates:", [str(x) for x in _cv_script_hits[:5]])
if not _cv_script_hits:
    raise FileNotFoundError("pantanal_infer_only_submission.py not found under /kaggle/input")
_cv_script_path = _cv_script_hits[0]
_cv_spec = _cv_importlib_util.spec_from_file_location("cv9245_sidecar", _cv_script_path)
if _cv_spec is None or _cv_spec.loader is None:
    raise RuntimeError(f"could not import CV9245 script from {_cv_script_path}")
_cv = _cv_importlib_util.module_from_spec(_cv_spec)
_cv_spec.loader.exec_module(_cv)

_cv_fold_hits = sorted(Path("/kaggle/input").rglob("moe_p0.60_c0.25_r0.15_post_p0.45_fold*.pt"))
_cv_cnn_hits = sorted(Path("/kaggle/input").rglob("student_cnn_2025_plus_2026_nodistill_keepperch*.pt"))
print("CV9245 fold candidates:", [str(x) for x in _cv_fold_hits])
print("CV9245 CNN candidates:", [str(x) for x in _cv_cnn_hits[:5]])
if len(_cv_fold_hits) != 4:
    raise FileNotFoundError(f"expected 4 CV9245 fold weights, found {len(_cv_fold_hits)}")
if not _cv_cnn_hits:
...
for _fold_path in _cv_fold_hits:
    ...
    _m.load_state_dict(torch.load(_fold_path, map_location="cpu"))
    _m.eval()
    _cv_fold_models.append(_m)

_cv_cnn = _cv.StudentCNN(emb_dim=1536, n_classes=N_CLASSES).to("cpu")
_cv_cnn.load_state_dict(torch.load(_cv_cnn_hits[0], map_location="cpu"))
_cv_cnn.eval()
_cv_mel_tf = _cv.torchaudio.transforms.MelSpectrogram(
    sample_rate=_cv.SR, n_fft=1024, win_length=1024, hop_length=320,
    n_mels=128, f_min=20, f_max=14000,
)

_cv_n_files = len(test_paths)
if sc_te.shape[0] != _cv_n_files * N_WINDOWS or emb_te.shape[0] != _cv_n_files * N_WINDOWS:
    raise RuntimeError(
        f"CV9245 row mismatch: files={_cv_n_files}, sc_te={sc_te.shape}, emb_te={emb_te.shape}, windows={N_WINDOWS}"
    )
_cv_sc_f = sc_te.reshape(_cv_n_files, N_WINDOWS, N_CLASSES).astype(np.float32)
_cv_emb_f = emb_te.reshape(_cv_n_files, N_WINDOWS, -1).astype(np.float32)
if _cv_emb_f.shape[-1] != 1536:
    raise RuntimeError(f"CV9245 expected 1536-d Perch embeddings, got {_cv_emb_f.shape}")

_cv_rows, _cv_preds = [], []
_cv_t0 = time.time()
for _fi, _path in enumerate(test_paths):
    _windows = _cv.read_60s_windows(Path(_path))
    _t_logits = _cv_sc_f[_fi].astype(np.float32)
    with torch.no_grad():
        _mel = torch.log(_cv_mel_tf(torch.from_numpy(_windows)) + 1e-6).unsqueeze(1)
        _, _cnn_logits = _cv_cnn(_mel)
        _cnn_logits = _cnn_logits.numpy().astype(np.float32)
    _t_logits = ((1.0 - CV9245_CNN_WEIGHT) * _t_logits + CV9245_CNN_WEIGHT * _cnn_logits).astype(np.float32)

    _site = _cv.parse_site(Path(_path).name)
    _hour = _cv.parse_hour(Path(_path).name)
    _sites_flat = np.array([_site] * N_WINDOWS)
    _hours_flat = np.array([_hour] * N_WINDOWS, dtype=np.int64)
    _prior = _cv.prior_logits_from_tables(_sites_flat, _hours_flat, _cv_prior_tables)
    _t_logits_use = _t_logits + CV9245_PRIOR_SCALE * _prior

    _x = torch.tensor(_cv_emb_f[_fi:_fi + 1], dtype=torch.float32)
    _l = torch.tensor(_t_logits_use[None, :, :], dtype=torch.float32)
    _sid = torch.tensor([_cv_site_to_idx.get(_site, 0)], dtype=torch.long)
    _hid = torch.tensor([_hour], dtype=torch.long)
    _fold_probs = []
    with torch.no_grad():
        for _m in _cv_fold_models:
            _out = _m(_x, _l, _sid, _hid)
            _fold_probs.append(torch.sigmoid(_out).cpu().numpy()[0])
    _probs = np.mean(_fold_probs, axis=0).astype(np.float32)
    _probs = _cv.postprocess_probs_filewise(_probs, n_windows=N_WINDOWS)
    _cv_preds.append(_probs)
    _stem = Path(_path).stem
    _cv_rows.extend([f"{_stem}_{(i + 1) * WINDOW_SEC}" for i in range(N_WINDOWS)])
    print(f"CV9245 sidecar: {_fi + 1}/{_cv_n_files} files, elapsed={time.time() - _cv_t0:.1f}s")

_cv_preds_arr = np.concatenate(_cv_preds, axis=0).astype(np.float32)
if not np.isfinite(_cv_preds_arr).all():
    raise ValueError("CV9245 sidecar predictions contain NaN/Inf")
_cv_sub = pd.DataFrame(np.clip(_cv_preds_arr, 0.0, 1.0), columns=PRIMARY_LABELS)
_cv_sub.insert(0, "row_id", _cv_rows)
_cv_sub.to_csv("submission_cv9245_cnnonly_sharedperch.csv", index=False)
print(
    f"CV9245 sidecar complete: submission_cv9245_cnnonly_sharedperch.csv {_cv_sub.shape}, "
    f"prob_range=({_cv_preds_arr.min():.6f},{_cv_preds_arr.max():.6f}), "
    f"mean={_cv_preds_arr.mean():.6f}, weight={CV9245_RANK_BLEND}, "
    f"runtime={time.time() - _cv_t0:.1f}s"
)
...
CV9245_CSV   = "submission_cv9245_cnnonly_sharedperch.csv"
...
rank_cv9245 = pd.DataFrame(p_cv9245).rank(axis=0, pct=True).to_numpy(np.float32)

print("Executing v548 2-way public946 rank blend plus 0.5% CV9245 sidecar...")
...
if not np.isfinite(rank_cv9245).all():
    raise ValueError("CV9245 rank predictions contain NaN/Inf")
pred = (1.0 - CV9245_RANK_BLEND) * pred + CV9245_RANK_BLEND * rank_cv9245
print(f"Applied CV9245 rank sidecar blend: weight={CV9245_RANK_BLEND}")
...
print("v548 CV9245 0.5% blend and post-processing complete. Ready for submission!")
