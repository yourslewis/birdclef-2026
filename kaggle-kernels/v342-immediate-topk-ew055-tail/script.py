#!/usr/bin/env python3
"""
BirdCLEF 2026 - v342 Immediate Top-k + Tail Damp Kernel
=============================================================================
...
print('V305: immediate-only temporal smoothing + top-k confidence contrast K=5')

# === Cell: Settings ===
# All high-impact knobs are centralized here so experiments can be audited.
# v320 keeps the known-strong v111/v118 family of settings, especially:
# - ProtoSSM ensemble weight = 0.55 (v342: carry v263 tied-best lower temporal weight into immediate top-k + tail damp)
...
POWER_GAMMA = 0.85
FILE_CONTEXT_ALPHA = 0.20
QUANTILE_MIX_ALPHA = 0.5

# v305 post-processing: combine two tied-best axes. v250 immediate-only
# temporal smoothing retained 0.926, and v255/v256 top-k contrast retained
# 0.926. This tests whether they are complementary.
TOPK_CONTRAST_K = 5
TOPK_BOOST_POWER = 0.90
NON_TOP_DAMP_POWER = 1.10

# v320: additionally apply v257 safe/tied-best gentle low-rank tail dampening
# after the v305 immediate top-k contrast. This tests whether BirdCLEF 2025
# power-adjustment ideas are complementary when both top-vs-rest and deeper
# tail calibration are used together.
TAIL_KEEP_TOP_K = 10
TAIL_DAMP_POWER = 1.05
...
# v342: row-wise top-k contrast after immediate-only smoothing and original
...
# v342: deeper-tail dampening from v257. Keep the top 10 labels after
# top-k contrast and gently suppress the remaining low-rank tail.
if TAIL_KEEP_TOP_K > 0 and TAIL_KEEP_TOP_K < probs.shape[1]:
    keep_idx = np.argpartition(probs, -TAIL_KEEP_TOP_K, axis=1)[:, -TAIL_KEEP_TOP_K:]
    keep_mask = np.zeros_like(probs, dtype=bool)
    rows = np.arange(probs.shape[0])[:, None]
    keep_mask[rows, keep_idx] = True
    probs = np.where(keep_mask, probs, np.power(probs, TAIL_DAMP_POWER))
    probs = np.clip(probs, 1e-8, 1.0 - 1e-8)
...
print(f'  Tail dampening: keep_top_k={TAIL_KEEP_TOP_K}, damp_power={TAIL_DAMP_POWER}')
print(f'  Post: immediate temporal smooth, sigmoid, file-context boost, gamma 0.85, top-k contrast, ProtoSSM EW0.55, tail dampening')
