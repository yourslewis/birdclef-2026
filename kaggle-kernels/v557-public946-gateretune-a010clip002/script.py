#!/usr/bin/env python3
"""
BirdCLEF 2026 v557 clipped low-displacement public946 gate-retune candidate.
...
print("Executing v557 clipped gate-retune rank blend (v542 baseline + clipped 10% retune)...")
V554_PROTO_WEIGHT = 0.56
V554_FAKE_BOOST = 0.04
V554_CTX_THR = 0.90
V554_CTX_BOOST = 0.10
V554_SED_RANK_THR = 0.93
V554_SED_BOOST = 0.08
V554_RARE_SCALE = 0.85
pred = (rank_proto * V554_PROTO_WEIGHT) + (rank_sed * (1.0 - V554_PROTO_WEIGHT))
...
pred = np.where(fake_only, (1.0 - V554_FAKE_BOOST) * pred + V554_FAKE_BOOST * rank_proto, pred)
...
proto_cont = (xctx > V554_CTX_THR) & (rank_proto > 0.75) & (p_sed < 0.12) & (~fake_only)
pred = np.where(proto_cont, (1.0 - V554_CTX_BOOST) * pred + V554_CTX_BOOST * np.maximum(rank_proto, xctx), pred)

sed_only = (rank_sed > V554_SED_RANK_THR) & (rank_proto < 0.80) & (~fake_only) & (~proto_cont)
pred = np.where(sed_only, (1.0 - V554_SED_BOOST) * pred + V554_SED_BOOST * rank_sed, pred)
...
try:
    ...
    for ci, species in enumerate(cols):
        if species in tax_df.index and tax_df.loc[species, "class_name"] in rare_classes:
            ...
            thr = vals.mean() + 0.05
            sub.iloc[:, col_idx] = np.where(vals < thr, vals * V554_RARE_SCALE, vals)
            rare_count += 1
...
# v557 risk-control: interpolate fully postprocessed v542 baseline with the
# stronger retune at low alpha, then clip per-cell movement. Observed gate on
# v556 outputs: alpha=0.10 plus clip=0.02 kept the same small AUC/top3 lift
# while capping max_abs at 0.02 and corr~0.99993.
V557_FULL_RETUNE_ALPHA = 0.10
V557_MAX_ABS_DELTA = 0.02

baseline_pred = (rank_proto * 0.60) + (rank_sed * 0.40)
baseline_fake_only = (p_proto > 0.50) & (p_sed < 0.05)
baseline_pred = np.where(baseline_fake_only, (1.0 - 0.08) * baseline_pred + 0.08 * rank_proto, baseline_pred)
baseline_proto_cont = (xctx > 0.88) & (rank_proto > 0.75) & (p_sed < 0.12) & (~baseline_fake_only)
baseline_pred = np.where(baseline_proto_cont, (1.0 - 0.15) * baseline_pred + 0.15 * np.maximum(rank_proto, xctx), baseline_pred)
baseline_sed_only = (rank_sed > 0.95) & (rank_proto < 0.80) & (~baseline_fake_only) & (~baseline_proto_cont)
baseline_pred = np.where(baseline_sed_only, (1.0 - 0.12) * baseline_pred + 0.12 * rank_sed, baseline_pred)

baseline_sub = df_proto.copy()
baseline_sub[cols] = baseline_pred.astype(np.float32)
for group in MIRROR_PAIRS:
    valid_idx = [col_to_idx[s] for s in group if s in col_to_idx]
    if len(valid_idx) >= 2:
        group_max = baseline_sub[cols].iloc[:, valid_idx].max(axis=1).to_numpy(np.float32)
        for idx in valid_idx:
            baseline_sub.iloc[:, idx + 1] = group_max
try:
    tax_df_v555 = pd.read_csv(BASE / "taxonomy.csv").set_index("primary_label")
    rare_classes_v555 = {"Amphibia", "Mammalia", "Reptilia"}
    for ci, species in enumerate(cols):
        if species in tax_df_v555.index and tax_df_v555.loc[species, "class_name"] in rare_classes_v555:
            col_idx = ci + 1
            vals = baseline_sub.iloc[:, col_idx].to_numpy(np.float32)
            thr = vals.mean() + 0.05
            baseline_sub.iloc[:, col_idx] = np.where(vals < thr, vals * 0.9, vals)
except Exception as e:
    print(f"v557 baseline reconstruction rare thresholding skipped: {e}")

sub_values = sub[cols].to_numpy(np.float32)
base_values = baseline_sub[cols].to_numpy(np.float32)
retune_delta = V557_FULL_RETUNE_ALPHA * (sub_values - base_values)
retune_delta = np.clip(retune_delta, -V557_MAX_ABS_DELTA, V557_MAX_ABS_DELTA)
sub[cols] = (base_values + retune_delta).astype(np.float32)
print(f"v557 clipped interpolation alpha={V557_FULL_RETUNE_ALPHA}, max_abs_delta={V557_MAX_ABS_DELTA}")
...
print(f"v557 gate retune params: proto_weight={V554_PROTO_WEIGHT}, fake_boost={V554_FAKE_BOOST}, ctx_thr={V554_CTX_THR}, ctx_boost={V554_CTX_BOOST}, sed_rank_thr={V554_SED_RANK_THR}, sed_boost={V554_SED_BOOST}, rare_scale={V554_RARE_SCALE}, alpha={V557_FULL_RETUNE_ALPHA}, max_abs_delta={V557_MAX_ABS_DELTA}")
print("v557 clipped low-displacement blend and post-processing complete. Ready for gated dry-run review!")
