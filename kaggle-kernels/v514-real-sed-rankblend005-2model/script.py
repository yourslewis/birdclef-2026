...
# v513: rank-blend real weak-label SED NFNet TorchScript predictions after
# existing v508 probability shaping. Public v510 tied 0.927 while stronger
# probability blend v511 dropped, so this keeps the safe 0.05 weight but uses
# per-class ranks to reduce calibration/scale mismatch.
REAL_SED_BLEND_WEIGHT = 0.05
REAL_SED_MAX_MODELS = 2
REAL_SED_MIN_MODELS = 2
REAL_SED_TIME_BUFFER_SEC = 15 * 60
REAL_SED_EST_SEC_PER_FILE_PER_MODEL = 1.15
REAL_SED_ZIP_NAME = 'sed-nfnet-v13v15-blend-v1.zip'
REAL_SED_DATASET_SLUG = 'bc26-sed-nfnet-v13v15-bundle-v1'
...
# === Cell: v514 Real SED 2-model rank blend ===
...
if REAL_SED_BLEND_WEIGHT > 0:
    try:
        ...
        if sed_probs_real is not None and sed_probs_real.shape == probs.shape:
            probs = rank_average_ensemble(
                [probs, sed_probs_real],
                weights=[1.0 - REAL_SED_BLEND_WEIGHT, REAL_SED_BLEND_WEIGHT],
            )
            probs = np.clip(probs, 1e-8, 1.0 - 1e-8)
            print(f'Applied real SED rank blend: weight={REAL_SED_BLEND_WEIGHT}')
        else:
...
print(f'  Post: v508 immediate temporal/logit local-event axis + real SED NFNet v13/v15 2-model per-class rank blend weight 0.05')
