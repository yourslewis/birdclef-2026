...
# v529: combine the public-best v517 softer taxon-gated axis with the
# OOF-calibrated B0 SED v23+v26 bundle. The v23/v26 OOF grid selected
# v23=0.40, v26=0.60 and improved local macro AUC over either member.
REAL_SED_BLEND_WEIGHT = 0.05
REAL_SED_MAX_MODELS = 3
REAL_SED_MIN_MODELS = 1
REAL_SED_TIME_BUFFER_SEC = 15 * 60
REAL_SED_EST_SEC_PER_FILE_PER_MODEL = 0.75
REAL_SED_ZIP_NAME = 'sed-b0-v23v26-oofblend040060-v1.zip'
REAL_SED_DATASET_SLUG = 'bc26-sed-b0-v23v26-oofblend040060-v1'
...
# v23+v26 B0 bundle dataset, emits the same 5-second row IDs, blends
# probabilities, then applies the v517 taxon gate to the combined probabilities.
...
def _sed_find_manifest():
    candidates = [
        ...
        Path('/kaggle/input') / REAL_SED_DATASET_SLUG / 'sed-b0-v23v26-oofblend040060-v1' / 'sed_bundle_manifest.json',
        Path('/kaggle/working/sed_b0_v23v26_bundle/sed_bundle_manifest.json'),
    ]
    for c in candidates:
        if c.exists():
            return c
    ...
    for z in zip_candidates:
        if z.exists():
            out_dir = Path('/kaggle/working/sed_b0_v23v26_bundle')
...
print(f'  Taxon max gate: floor={TAXON_MAX_GATE_FLOOR}, alpha={TAXON_MAX_GATE_ALPHA}')
print(f'  Post: v508 immediate temporal/logit local-event axis + OOF-calibrated B0 v23/v26 SED blend weight {REAL_SED_BLEND_WEIGHT}, then v517 taxon max gate')
