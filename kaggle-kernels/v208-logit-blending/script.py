...
# Ensemble averaging on logit scale
def logit_average(logits_list, weights=None):
    import numpy as np
    # logits_list: list of numpy arrays containing raw logits before sigmoid/softmax
    if weights is None:
    ...
    avg_logits = np.zeros_like(logits_list[0])
    for w, l in zip(weights, logits_list):
        avg_logits += w * l
        
    # Apply sigmoid after blending
    return 1.0 / (1.0 + np.exp(-avg_logits))
