#!/usr/bin/env python3
"""
BirdCLEF 2026 — v240 Top-K Contrast Kernel
=============================================================================
...
- Target behavior: improve over the current 0.923 LB anchor by combining v111/v118
  strengths plus stronger within-file context boost, higher ProtoSSM weight, and a light row-wise top-k contrast post-processing step.
"""
...
print('V240: v239 + light top-k confidence contrast post-processing')

# === Cell: Settings ===
# All high-impact knobs are centralized here so experiments can be audited.
# v240 keeps the known-strong v111/v118 family of settings, especially:
...
# v240 post-processing: lightly emphasize each row's top candidates and dampen
# the long tail. This follows the BirdCLEF-style intuition that each 5-second
# window usually contains only a few salient species/classes, while diffuse low
# probabilities tend to be false positives. Keep this mild to avoid destroying
# macro-AUC rank order.
TOPK_CONTRAST_K = 3
TOPK_BOOST_POWER = 0.95
NON_TOP_DAMP_POWER = 1.05
...
# v240: row-wise top-k contrast. Boost only the few strongest labels in each
# window and gently dampen the rest. Power < 1 boosts; power > 1 suppresses.
# This is intentionally after the global power calibration so it acts as a final
# per-window sparsity prior rather than changing upstream model logits.
