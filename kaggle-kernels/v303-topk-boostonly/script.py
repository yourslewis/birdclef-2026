#!/usr/bin/env python3
"""
BirdCLEF 2026 - v303 Top-k Boost-only Contrast Kernel
=============================================================================
...
- Target behavior: improve over the current 0.923 LB anchor by combining v111/v118
  strengths plus top-5 boost-only row-wise confidence contrast.
"""
...
print('V303: v245 smoothing + top-k boost-only contrast K=5 p0.85/1.00')

# === Cell: Settings ===
# All high-impact knobs are centralized here so experiments can be audited.
# v303 keeps the known-strong v111/v118 family of settings, especially:
...
# v303 post-processing: isolate the top-boost side of BirdCLEF-style power
# adjustment. v302 strengthened both top boost and non-top damp; v303 keeps
# K=5 and original gamma but boosts top predictions only, avoiding extra tail damp.
TOPK_CONTRAST_K = 5
...
# v303: top-k boost-only contrast after original gamma scaling.
# Boost top 5 labels per row but leave non-top labels unchanged.
...
print(f'  Post: Gaussian smooth, sigmoid, file-context boost, gamma 0.85, top-k boost-only')
