...
# v519: blend in a stronger 5s EfficientNet-B0 pseudo-label student initialized from 12-epoch XC pretraining after
# existing v508 probability shaping. Local labeled-soundscape sweep favors a
# larger local-optimal linear blend (0.15) because the ep12-init student improved standalone and labeled-soundscape blend.
REAL_SED_BLEND_WEIGHT = 0.15
REAL_SED_MAX_MODELS = 1
REAL_SED_MIN_MODELS = 1
REAL_SED_TIME_BUFFER_SEC = 10 * 60
REAL_SED_EST_SEC_PER_FILE_PER_MODEL = 0.20
REAL_SED_ZIP_NAME = 'bc26-student-b0-5s-ep12init-v1.zip'
REAL_SED_DATASET_SLUG = 'bc26-student-b0-5s-ep12init-v1'
...
# === Cell: v519 Student B0 5s ep12-init bundle blend ===
# Load the zipped TorchScript pseudo-label student dataset, emit the same
# 5-second row IDs, and blend probabilities into the v508 axis.
...
def _sed_find_manifest():
    candidates = [
        ...
        Path('/kaggle/input') / REAL_SED_DATASET_SLUG / 'sed-nfnet-v13v15-blend-v1' / 'sed_bundle_manifest.json',
        Path('/kaggle/working/sed_nfnet_v13v15_bundle/sed_bundle_manifest.json'),
    ]
    for c in candidates:
        if c.exists():
            return c
    ...
    for z in zip_candidates:
        if z.exists():
            out_dir = Path('/kaggle/working/sed_nfnet_v13v15_bundle')
...
def _sed_infer_bundle(test_paths, target_row_ids):
    ...
    if not entries:
        return None
    print(f'Loading {len(entries)}/{len(manifest["models"])} real SED TorchScript models')
    ...
    for entry in entries:
        ...
        models.append((entry, torch.jit.load(str(model_path), map_location=device).eval()))
    weight_sum = sum(float(e.get('weight', 0.0)) for e, _ in models)
    if weight_sum <= 0:
        ...
        with torch.no_grad():
            for entry, model in models:
                ...
                for start in range(0, len(x_all), 4):
                    clip_logits, _frame_logits = model(x_all[start:start + 4])
...
print(f'  Post: v508 immediate temporal/logit local-event axis + 5s B0 ep12 external-init pseudo-label student blend weight 0.15')
