#!/usr/bin/env python3
"""
BirdCLEF 2026 - v304 Top-k Damp-only Contrast Kernel
=============================================================================
...
- Target behavior: improve over the current 0.923 LB anchor by combining v111/v118
  strengths plus non-top damp-only row-wise confidence contrast.
"""
...
print('V304: v245 smoothing + top-k damp-only contrast K=5 p1.00/1.15')

# === Cell: Settings ===
# All high-impact knobs are centralized here so experiments can be audited.
# v304 keeps the known-strong v111/v118 family of settings, especially:
...
# v304 post-processing: isolate the non-top damp side of BirdCLEF-style power
# adjustment. v303 tested top-boost only and v302 tested boost+damp; v304 keeps
# K=5 and original gamma but dampens non-top predictions only.
TOPK_CONTRAST_K = 5
...
# v304: non-top damp-only contrast after original gamma scaling.
# Leave top 5 labels unchanged and damp all remaining labels.
...
print(f'  Post: Gaussian smooth, sigmoid, file-context boost, gamma 0.85, non-top damp-only')
