...
def get_balanced_sampler(df):
    # Count frequencies of primary_label
    class_counts = df['primary_label'].value_counts().to_dict()
    # Total samples
    total = len(df)
    
    # Calculate weight for each sample: 1.0 / frequency of its class
    weights = []
    ...
    # Create the sampler (sample with replacement so rare classes appear more)
