...
# %% markdown
# ## v550 Snowflake SED Sidecar
#
# Source-clean v550 addition: run Tsubasa Kanno's public Snowflake SED ONNX
# sidecar (ConvNeXt-Tiny + TF-EfficientNetV2-M, one fold each) on the same
# 5-second windows.  The sidecar is written as `submission_snowflake_sed.csv`
# so it can be gated offline before any competition submission slot is spent.

# %% cell v550_snowflake_sed_sidecar
print("Starting v550 Snowflake SED sidecar...")
SNOWFLAKE_CSV = "submission_snowflake_sed.csv"
SNOWFLAKE_RANK_BLEND = 0.01

def _find_snowflake_paths():
    conv = sorted(Path("/kaggle/input").rglob("sed_convnext-tiny_fold0.onnx"))
    eff = sorted(Path("/kaggle/input").rglob("sed_tf-efficientnetv2-m_fold0.onnx"))
    print("Snowflake ConvNeXt candidates:", [str(x) for x in conv[:5]])
    print("Snowflake EfficientNetV2 candidates:", [str(x) for x in eff[:5]])
    if not conv or not eff:
        raise FileNotFoundError("Snowflake SED ONNX models not found; attach tsubasatech/birdclef-2026-snowflake-sed")
    return [conv[0], eff[0]]

def _make_snowflake_session(path):
    so = ort.SessionOptions()
    so.intra_op_num_threads = 4
    so.inter_op_num_threads = 1
    so.graph_optimization_level = ort.GraphOptimizationLevel.ORT_ENABLE_ALL
...
_snow_paths = _find_snowflake_paths()
_snow_sessions = [_make_snowflake_session(p) for p in _snow_paths]
for _p, _sess in zip(_snow_paths, _snow_sessions):
    print(
        "Snowflake model", _p,
...
_snow_rows, _snow_preds = [], []
_snow_t0 = time.time()
for _i, _path in enumerate(test_paths, 1):
    _chunks, _ends = file_to_sed_chunks(_path)
    _audio = _chunks.astype(np.float32)
    _p_sum = np.zeros((_audio.shape[0], N_CLASSES), dtype=np.float32)
    for _sess in _snow_sessions:
        _input = _sess.get_inputs()[0]
        _x = _audio
        # Public Snowflake kernels feed raw 5-second audio chunks.  Keep a
        # small shape adapter for any ONNX export that declares an extra unit
        # channel dimension.
        _shape = list(_input.shape)
        if len(_shape) == 3 and _x.ndim == 2:
            _x = _x[:, None, :]
        _outs = _sess.run(None, {_input.name: _x.astype(np.float32)})
        _logits = _outs[0]
        if _logits.ndim == 3:
            _logits = _logits.max(axis=1)
        if _logits.shape[-1] != N_CLASSES:
            raise RuntimeError(f"Snowflake output class mismatch: got {_logits.shape}, expected last dim {N_CLASSES}")
        _p_sum += sigmoid_sed(_logits.astype(np.float32))
    _p_mean = _p_sum / float(len(_snow_sessions))
    if len(_p_mean) > 1:
        _p_mean = gaussian_filter1d(_p_mean, sigma=0.65, axis=0, mode="nearest").astype(np.float32)
    _stem = _path.stem
    _snow_rows.extend([f"{_stem}_{int(t)}" for t in _ends])
    _snow_preds.append(_p_mean.astype(np.float32))
    if _i == 1 or _i % 50 == 0 or _i == len(test_paths):
        print(f"Snowflake SED: {_i}/{len(test_paths)}, elapsed={time.time() - _snow_t0:.1f}s")

_snow_arr = np.concatenate(_snow_preds, axis=0).astype(np.float32)
if not np.isfinite(_snow_arr).all():
    raise ValueError("Snowflake sidecar predictions contain NaN/Inf")
_snow_sub = pd.DataFrame(np.clip(_snow_arr, 0.0, 1.0), columns=PRIMARY_LABELS)
_snow_sub.insert(0, "row_id", _snow_rows)
_snow_sub.to_csv(SNOWFLAKE_CSV, index=False)
print(
    f"Snowflake sidecar complete: {SNOWFLAKE_CSV} {_snow_sub.shape}, "
    f"prob_range=({_snow_arr.min():.6f},{_snow_arr.max():.6f}), "
    f"mean={_snow_arr.mean():.6f}, weight={SNOWFLAKE_RANK_BLEND}, "
    f"runtime={time.time() - _snow_t0:.1f}s"
)
...
rank_snowflake = pd.DataFrame(p_snowflake).rank(axis=0, pct=True).to_numpy(np.float32)

print("Executing v550 2-way public946 rank blend plus 1% Snowflake SED sidecar...")
...
pred = (1.0 - SNOWFLAKE_RANK_BLEND) * pred + SNOWFLAKE_RANK_BLEND * rank_snowflake
pred = np.clip(pred, 0.0, 1.0).astype(np.float32)
print(f"Applied Snowflake SED rank sidecar blend: weight={SNOWFLAKE_RANK_BLEND}")
