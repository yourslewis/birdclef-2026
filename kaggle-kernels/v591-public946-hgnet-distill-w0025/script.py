...
HGNET_DEVICE = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
print(f'v591 HGNet sidecar using weights from {HGNET_WEIGHT_DIR}; device={HGNET_DEVICE}')
...
def _load_segment(audio_id: str, end_sec: int, audio_cache: dict, is_dry_run: bool):
    if audio_id not in audio_cache:
        ...
        if path is None:
            if is_dry_run:
                audio_cache[audio_id] = (np.zeros(HGNET_SR * HGNET_SEGMENT_SEC, dtype=np.float32), HGNET_SR)
            else:
...
_lms = HGNetLogMelSpectrogramTransform().eval().to(HGNET_DEVICE)
_fold_probs = []
with torch.no_grad():
    for fold in range(4):
        ...
        for start in range(0, len(_waves), HGNET_BATCH):
            batch = _waves[start:start + HGNET_BATCH].to(HGNET_DEVICE, non_blocking=True)
            lms = _lms(batch)
