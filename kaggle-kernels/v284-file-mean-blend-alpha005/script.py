#!/usr/bin/env python3
"""
BirdCLEF 2026 — v284 Immediate Temporal + Ultra-Gentle File Mean Blend Kernel
=============================================================================
...
- Target behavior: improve over the current 0.923 LB anchor by combining v111/v118
  strengths plus immediate-neighbor temporal smoothing and ultra-gentle BirdCLEF-2024-style within-file mean blending.
"""
...
print('V284: immediate-only temporal smoothing + file mean blend context alpha 0.05')

# === Cell: Settings ===
# All high-impact knobs are centralized here so experiments can be audited.
# v284 keeps the known-strong v111/v118 family of settings, especially:
...
def file_context_boost(probs, alpha=FILE_CONTEXT_ALPHA):
    ...
    if n_complete_files > 0:
        ...
        # BirdCLEF 2024 6th-place post-processing blended each class time-series
        # with its per-file mean after temporal smoothing. v284 uses alpha=0.05,
        # an ultra-gentle variant intended to preserve sharp 5-second detections
        # while adding a small repeated-soundscape consistency prior.
...
print(f'  Post: Gaussian smooth → sigmoid → ultra-gentle file-mean blend → power transform')
