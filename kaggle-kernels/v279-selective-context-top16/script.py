#!/usr/bin/env python3
"""
BirdCLEF 2026 — v279 Selective Context Top16 Kernel
=============================================================================
...
- Target behavior: improve over the current 0.923 LB anchor by combining v111/v118
  strengths plus immediate-neighbor temporal smoothing and tighter top-16 file-context boosting.
"""
...
print('V279: immediate-only temporal smoothing + selective top-16 file-context boost')

# === Cell: Settings ===
# All high-impact knobs are centralized here so experiments can be audited.
# v279 keeps the known-strong v111/v118 family of settings, especially:
...
def file_context_boost(probs, alpha=FILE_CONTEXT_ALPHA, topk=FILE_CONTEXT_TOPK):
    if alpha <= 0:
        return probs
    ...
    if n_complete_files > 0:
        ...
        boosted_all = (1.0 - alpha) * view + alpha * file_max
        if topk is not None and 0 < topk < probs.shape[1]:
            # Apply within-file evidence sharing only to the strongest candidate
            # species per soundscape. Top-16 is a stricter gate than v278 top-32,
            # intended to reduce long-tail false-positive lifts while retaining
            # repeated-species temporal context for likely classes.
            top_idx = np.argpartition(file_max[:, 0, :], -topk, axis=1)[:, -topk:]
            mask = np.zeros_like(file_max, dtype=bool)
            np.put_along_axis(mask[:, 0, :], top_idx, True, axis=1)
            boosted = np.where(mask, boosted_all, view)
        else:
            boosted = boosted_all
...
print(f'  Power gamma={POWER_GAMMA}, File context alpha={FILE_CONTEXT_ALPHA}, topk={FILE_CONTEXT_TOPK}')
