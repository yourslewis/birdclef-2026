#!/usr/bin/env python3
"""
BirdCLEF 2026 - v307 Immediate Temporal + Non-top Damp-only Kernel
=============================================================================
...
- Target behavior: improve over the current 0.923 LB anchor by combining v111/v118
  strengths plus immediate-only temporal smoothing combined with non-top damp-only confidence contrast.
"""
...
print('V307: immediate-only temporal smoothing + non-top damp-only K=5 p1.00/1.15')

# === Cell: Settings ===
# All high-impact knobs are centralized here so experiments can be audited.
# v307 keeps the known-strong v111/v118 family of settings, especially:
...
# v307 post-processing: combine v250 immediate-only temporal smoothing with
# v304 non-top damp-only contrast. This tests whether suppressing low-rank
# labels helps on the immediate smoothing axis without boosting top labels.
TOPK_CONTRAST_K = 5
TOPK_BOOST_POWER = 1.00
NON_TOP_DAMP_POWER = 1.15
...
# v307: non-top damp-only contrast after immediate-only smoothing and original
# gamma scaling. Leave top 5 labels unchanged and damp all remaining labels.
...
print(f'  Top-k damp-only: k={TOPK_CONTRAST_K}, top={TOPK_BOOST_POWER}, damp={NON_TOP_DAMP_POWER}')
print(f'  Post: immediate temporal smooth, sigmoid, file-context boost, gamma 0.85, non-top damp-only')
