#!/usr/bin/env python3
"""
BirdCLEF 2026 — v317 v257 Smoothing + Tail Dampening Kernel
=============================================================================
...
- Target behavior: improve over the current 0.923 LB anchor by combining v111/v118
  strengths plus immediate-only temporal smoothing and row-wise low-rank tail dampening.
"""
...
print('V317: immediate-only temporal smoothing + low-rank tail dampening K10 p1.05')
...
# v257 post-processing: keep the strongest labels in each row unchanged and
# damp the low-rank tail. Inspired by BirdCLEF 2025 power-adjustment summaries
# that describe boosting top confident predictions and lowering others.
TAIL_KEEP_TOP_K = 10
...
# v317: row-wise low-rank tail dampening. Keep top-K class probabilities per
...
print(f'  Post: immediate temporal smooth → sigmoid → file-context boost → gamma 0.85 → tail dampening')
