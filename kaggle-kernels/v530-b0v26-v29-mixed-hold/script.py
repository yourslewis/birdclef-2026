...
REAL_SED_EST_SEC_PER_FILE_PER_MODEL = 0.95
REAL_SED_ZIP_NAME = 'sed-b0v26-plus-nfnet-v29-oofblend090010-v1.zip'
REAL_SED_DATASET_SLUG = 'bc26-sed-b0v26-nfnet-v29-oofblend090010-v1'
...
def _sed_find_manifest():
    candidates = [
        ...
        Path('/kaggle/input') / REAL_SED_DATASET_SLUG / 'sed-b0v26-plus-nfnet-v29-oofblend090010-v1' / 'sed_bundle_manifest.json',
        Path('/kaggle/working/sed_mixed_b0v26_v29_bundle/sed_bundle_manifest.json'),
    ]
    for c in candidates:
        if c.exists():
            return c
    ...
    for z in zip_candidates:
        if z.exists():
            out_dir = Path('/kaggle/working/sed_mixed_b0v26_v29_bundle')
...
def _sed_audio_config_key(audio_cfg):
...
def _sed_infer_bundle(test_paths, target_row_ids):
    ...
    print('Real SED bundle mixed_audio_config=' + str(manifest.get('mixed_audio_config', False)))
    ...
    for path_i, path in enumerate(test_paths):
        # Mixed bundles can combine models trained with different context length
        # and mel resolution. Cache decoded audio by sample rate and generated
        # row windows by full audio_config so B0 10s/160mel and NFNet
        # 20s/128mel models can run safely in one Kaggle kernel.
        audio_cache = {}
        x_cache = {}
        ...
        with torch.no_grad():
            for entry, model in models:
                ...
                key = _sed_audio_config_key(audio_cfg)
                if key not in x_cache:
                    ...
                    windows = [
                        _sed_waveform_to_logmel(
                            torch.from_numpy(_sed_extract_context(audio, sr, end_sec, context_sec).copy()),
                            audio_cfg,
                            mel_fb,
                        )
