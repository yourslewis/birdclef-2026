#!/usr/bin/env python3
"""
BirdCLEF 2026 — v281 Immediate Temporal + File Mean Blend Kernel
=============================================================================
...
- Target behavior: improve over the current 0.923 LB anchor by combining v111/v118
  strengths plus immediate-neighbor temporal smoothing and BirdCLEF-2024-style within-file mean blending.
"""
...
print('V281: immediate-only temporal smoothing + file mean blend context')

# === Cell: Settings ===
# All high-impact knobs are centralized here so experiments can be audited.
# v281 keeps the known-strong v111/v118 family of settings, especially:
...
def file_context_boost(probs, alpha=FILE_CONTEXT_ALPHA):
    ...
    if n_complete_files > 0:
        ...
        # with its per-file mean after temporal smoothing. This is gentler than
        # max-based file context: it dampens isolated spikes and raises classes
        # that have repeated support throughout the 60-second soundscape.
...
print(f'  Post: Gaussian smooth → sigmoid → file-mean blend → power transform')
