...
# v526: combine the public-best v516 taxon-gated axis with the newly staged
# v23d 20s/128-mel NFNet sidecar. v23d is lower-correlated than v22b/v13/v15
# on the fair OOF overlap, so keep the first public bridge conservative.
REAL_SED_BLEND_WEIGHT = 0.05
REAL_SED_MAX_MODELS = 3
REAL_SED_MIN_MODELS = 1
REAL_SED_TIME_BUFFER_SEC = 15 * 60
REAL_SED_EST_SEC_PER_FILE_PER_MODEL = 1.15
REAL_SED_ZIP_NAME = 'sed-nfnet-v23d-20s128-lowcorr-v1.zip'
REAL_SED_DATASET_SLUG = 'bc26-sed-nfnet-v23d-20s128-lowcorr-v1'
...
# v23d bundle dataset, emits the same 5-second row IDs, blends probabilities,
# then applies the v516 taxon gate to the combined probabilities.
...
def _sed_find_manifest():
    candidates = [
        ...
        Path('/kaggle/input') / REAL_SED_DATASET_SLUG / 'sed-nfnet-v23d-20s128-lowcorr-v1' / 'sed_bundle_manifest.json',
        Path('/kaggle/working/sed_nfnet_v23d_bundle/sed_bundle_manifest.json'),
    ]
    for c in candidates:
        if c.exists():
            return c
    ...
    for z in zip_candidates:
        if z.exists():
            out_dir = Path('/kaggle/working/sed_nfnet_v23d_bundle')
...
print(f'  Taxon max gate: floor={TAXON_MAX_GATE_FLOOR}, alpha={TAXON_MAX_GATE_ALPHA}')
print(f'  Post: v508 immediate temporal/logit local-event axis + conservative v23d 20s/128 NFNet blend weight {REAL_SED_BLEND_WEIGHT}, then v516 taxon max gate')
