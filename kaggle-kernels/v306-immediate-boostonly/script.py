#!/usr/bin/env python3
"""
BirdCLEF 2026 - v306 Immediate Temporal + Top-k Boost-only Kernel
=============================================================================
...
- Target behavior: improve over the current 0.923 LB anchor by combining v111/v118
  strengths plus immediate-only temporal smoothing combined with top-5 boost-only confidence contrast.
"""
...
print('V306: immediate-only temporal smoothing + top-k boost-only K=5 p0.85/1.00')

# === Cell: Settings ===
# All high-impact knobs are centralized here so experiments can be audited.
# v306 keeps the known-strong v111/v118 family of settings, especially:
...
# v306 post-processing: combine v250 immediate-only temporal smoothing with
# v303 top-boost-only contrast. This tests whether confidence boosting helps
# on the immediate smoothing axis without extra non-top dampening.
TOPK_CONTRAST_K = 5
TOPK_BOOST_POWER = 0.85
NON_TOP_DAMP_POWER = 1.00
...
# v306: top-k boost-only contrast after immediate-only smoothing and original
# gamma scaling. Boost top 5 labels per row and leave non-top labels unchanged.
...
print(f'  Top-k boost-only: k={TOPK_CONTRAST_K}, boost={TOPK_BOOST_POWER}, non_top={NON_TOP_DAMP_POWER}')
print(f'  Post: immediate temporal smooth, sigmoid, file-context boost, gamma 0.85, top-k boost-only')
