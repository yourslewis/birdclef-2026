...
def add_background_noise(audio, noise_level=0.05, p=0.5):
    # audio is (channels, time)
    if np.random.rand() > p:
        return audio
        
    # Generate pink noise (1/f noise) approximation
    # Pink noise better mimics environmental elements like water and wind
    noise = np.random.randn(*audio.shape)
    # Simple lowpass filter integration to mimic pink
    for i in range(1, noise.shape[-1]):
        noise[..., i] = 0.99 * noise[..., i-1] + 0.01 * noise[..., i]
        
    # Normalize noise
    noise = noise / np.max(np.abs(noise))
    
    # Scale relative to audio max amplitude
    audio_max = np.max(np.abs(audio)) if np.max(np.abs(audio)) > 0 else 1.0
    
    return audio + (noise * noise_level * audio_max)

# We assume this is called inside the dataset __getitem__ function before spectrogram conversion
