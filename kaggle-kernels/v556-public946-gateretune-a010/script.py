#!/usr/bin/env python3
"""
BirdCLEF 2026 v556 low-displacement public946 gate-retune interpolation candidate.
...
print("Executing v556 low-displacement gate-retune rank blend (v542 baseline + 10% retune)...")
...
# v556 risk-control: interpolate fully postprocessed v542 baseline with the
# stronger retune at a lower alpha. Observed v555 backoff: alpha=0.10 kept
# a small dry-run AUC/top3 lift while reducing displacement to corr~0.99961,
# MAE~0.00177, max_abs~0.120.
V556_FULL_RETUNE_ALPHA = 0.10
...
try:
    ...
    print(f"v556 baseline reconstruction rare thresholding skipped: {e}")

sub_values = sub[cols].to_numpy(np.float32)
base_values = baseline_sub[cols].to_numpy(np.float32)
sub[cols] = ((1.0 - V556_FULL_RETUNE_ALPHA) * base_values + V556_FULL_RETUNE_ALPHA * sub_values).astype(np.float32)
print(f"v556 low-displacement interpolation alpha={V556_FULL_RETUNE_ALPHA}")
...
print(f"v556 gate retune params: proto_weight={V554_PROTO_WEIGHT}, fake_boost={V554_FAKE_BOOST}, ctx_thr={V554_CTX_THR}, ctx_boost={V554_CTX_BOOST}, sed_rank_thr={V554_SED_RANK_THR}, sed_boost={V554_SED_BOOST}, rare_scale={V554_RARE_SCALE}, alpha={V556_FULL_RETUNE_ALPHA}")
print("v556 low-displacement blend and post-processing complete. Ready for gated dry-run review!")
