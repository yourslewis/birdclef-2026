...
def temporal_smoothing(predictions, kernel_size=3):
    # predictions: array of shape (N_windows, num_classes)
    # Applies a simple moving average filter along the time axis
    smoothed = np.copy(predictions)
    pad_w = kernel_size // 2
    
    padded = np.pad(predictions, ((pad_w, pad_w), (0, 0)), mode='edge')
    
    for i in range(len(smoothed)):
        # Mean across the temporal window for each class
        smoothed[i] = np.mean(padded[i:i+kernel_size], axis=0)
