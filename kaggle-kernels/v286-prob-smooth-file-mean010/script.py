#!/usr/bin/env python3
"""
BirdCLEF 2026 — v286 Probability Smooth + File Mean Blend Kernel
=============================================================================
...
- Target behavior: improve over the current 0.923 LB anchor by combining v111/v118
  strengths plus probability-space immediate temporal smoothing and gentle within-file mean blending.
"""
...
print('V286: probability temporal smoothing + file mean blend alpha 0.10')

# === Cell: Settings ===
# All high-impact knobs are centralized here so experiments can be audited.
# v286 keeps the known-strong v111/v118 family of settings, especially:
...
POWER_GAMMA = 0.85
FILE_CONTEXT_ALPHA = 0.10
FILE_CONTEXT_MODE = "mean"
...
def temporal_smooth(scores, weights=GAUSSIAN_WEIGHTS):
    n_windows = N_WINDOWS
...
def file_context_boost(probs, alpha=FILE_CONTEXT_ALPHA):
    ...
    if n_complete_files > 0:
        ...
        # Gentle BirdCLEF-2024-style within-file mean blend. Paired with
        # probability-space temporal smoothing in v286 so both post-processors
        # operate on predictions rather than pre-sigmoid logits.
        file_mean = view.mean(axis=1, keepdims=True)
        boosted = (1.0 - alpha) * view + alpha * file_mean
...
# Apply temporal smoothing and file-level context in probability space, matching
# BirdCLEF solution writeups that smooth predictions rather than raw logits.
probs = sigmoid(final_scores_tempered)
...
print(f'  Power gamma={POWER_GAMMA}, File context mode={FILE_CONTEXT_MODE}, alpha={FILE_CONTEXT_ALPHA}')
print(f'  Probes: {len(probe_models)} MLP + {len(protossm_models)}x ProtoSSM SSM')
print(f'  Post: sigmoid → probability temporal smooth → file-mean blend → power transform')
