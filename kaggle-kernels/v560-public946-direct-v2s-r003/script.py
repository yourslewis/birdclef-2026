...
# %% markdown
# ## v560 EfficientNetV2-S public946 direct student sidecar
#
# Source-clean v552 addition: load a private TorchScript EfficientNetV2-S student
# trained from the public946 sed85/rankblend15 blended teacher and write `submission_direct_v2s_student.csv`.
# The final blend applies this side stream as a conservative per-class rank sidecar.

# %% cell v552_direct_v2s_student_sidecar
print("Starting v560 EfficientNetV2-S public946 direct student sidecar...")
STUDENT_CSV = "submission_direct_v2s_student.csv"
STUDENT_RANK_BLEND = 0.03
APPLY_TAXON_MAX_GATE = False
TAXON_MAX_GATE_FLOOR = 0.30
TAXON_MAX_GATE_ALPHA = 0.50
STUDENT_DATASET_SLUG = "bc26-public946-direct-v2s-student-v1"
STUDENT_ZIP_NAME = "bc26-public946-direct-v2s-student-v1.zip"
...
def _student_find_manifest():
    candidates = [
        ...
        Path("/kaggle/working/direct_v2s_student_bundle/sed_bundle_manifest.json"),
    ]
    for c in candidates:
        if c.exists():
            return c
    ...
    for z in zips:
        if z.exists():
            out = Path("/kaggle/working/direct_v2s_student_bundle")
            out.mkdir(parents=True, exist_ok=True)
            manifest = out / "sed_bundle_manifest.json"
            if not manifest.exists():
                print(f"Extracting Direct V2S student bundle from {z} ...")
    ...
    raise FileNotFoundError("Direct V2S student bundle manifest not found")
...
def _student_infer_bundle(paths):
    ...
    print(f"Direct V2S student manifest: {manifest_path}")
    manifest = json.loads(manifest_path.read_text())
    labels = [str(x) for x in manifest["labels"]]
    audio_cfg = manifest["audio_config"]
    entries = manifest.get("models", [])
    if not entries:
        raise RuntimeError("Direct V2S student manifest has no models")
    ...
    for i, path in enumerate(paths, 1):
        ...
        if i == 1 or i % 10 == 0 or i == len(paths):
            print(f"Direct V2S student inferred {i}/{len(paths)} files; elapsed={time.time() - t0:.1f}s")
    ...
    print(f"Direct V2S student prob range: {vals.min():.6f} to {vals.max():.6f}, mean={vals.mean():.6f}; runtime={time.time() - t0:.1f}s")
    return pred

_student_sub = _student_infer_bundle(test_paths)
_student_sub.to_csv(STUDENT_CSV, index=False)
print(f"Direct V2S student sidecar complete: {STUDENT_CSV} {_student_sub.shape}, weight={STUDENT_RANK_BLEND}")
...
print(f"Applied Direct V2S student rank sidecar blend: weight={STUDENT_RANK_BLEND}")
