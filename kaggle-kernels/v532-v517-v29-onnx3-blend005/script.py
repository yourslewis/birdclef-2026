...
# v528: combine the public-best v517 softer taxon-gated axis with the newly staged
# v29 broad observed-class 20s/128-mel NFNet sidecar. v29 is low-correlated
# with v13/v15/v23d and improves OOF overlap blends at moderate weights.
REAL_SED_BLEND_WEIGHT = 0.05
REAL_SED_MAX_MODELS = 3
REAL_SED_MIN_MODELS = 1
REAL_SED_TIME_BUFFER_SEC = 30 * 60
REAL_SED_EST_SEC_PER_FILE_PER_MODEL = 1.00
REAL_SED_ZIP_NAME = 'sed-nfnet-v29-20s128-broad181-onnx-v1.zip'
REAL_SED_DATASET_SLUG = 'bc26-sed-nfnet-v29-20s128-onnx-v1'
...
def _sed_find_manifest():
    candidates = [
        ...
        Path('/kaggle/input') / REAL_SED_DATASET_SLUG / 'sed-nfnet-v29-20s128-broad181-onnx-v1' / 'sed_bundle_manifest.json',
        Path('/kaggle/working/sed_nfnet_v29_onnx_bundle/sed_bundle_manifest.json'),
    ]
    for c in candidates:
        if c.exists():
            return c
    ...
    for z in zip_candidates:
        if z.exists():
            out_dir = Path('/kaggle/working/sed_nfnet_v29_onnx_bundle')
...
if REAL_SED_BLEND_WEIGHT > 0:
    try:
        ...
        if sed_probs_real is not None and sed_probs_real.shape == probs.shape:
            ...
            probs = np.clip(probs, 1e-8, 1.0 - 1e-8)
            print(f'Applied real SED bundle blend: weight={REAL_SED_BLEND_WEIGHT}')
        else:
...
print(f'  Taxon max gate: floor={TAXON_MAX_GATE_FLOOR}, alpha={TAXON_MAX_GATE_ALPHA}')
print(f'  Post: v508 immediate temporal/logit local-event axis + ONNX3 v29 20s/128 broad NFNet blend weight {REAL_SED_BLEND_WEIGHT}, then v517 taxon max gate')
