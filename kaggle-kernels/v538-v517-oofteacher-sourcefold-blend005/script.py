#!/usr/bin/env python3
"""
BirdCLEF 2026 - v508 SED Local Logit Event Top3 Context0.275 Kernel
=============================================================================
...
import subprocess, sys, glob as _glob, zipfile
...
print('V508: SED local logit-event boost top3 + ProtoSSM EW0.625 + gamma 0.825 + context 0.275')

# === Cell: Settings ===
# All high-impact knobs are centralized here so experiments can be audited.
# v508 keeps the known-strong v111/v118 family of settings, especially:
...
# v508: SED-inspired local event propagation in logit space. v501-v508
# tied best at context0.25 while v500 context0.20 slipped; v508 keeps v508
# top-3 logit local events and probes a slightly stronger file context alpha
# of 0.275 for more whole-recording consistency.
LOCAL_EVENT_ALPHA = 0.05
LOCAL_EVENT_TOPK = 3

# v538: same compact fold-aware OOF-teacher B0 sidecar as v537, but with a moderately stronger 0.05 blend.  The source-fold OOF check
# reached 0.943801 on the B0v26+v29 intersection, but the model is B0-derived,
# so use a still-modest 5% blend into the current v517 taxon-gated axis.
REAL_SED_BLEND_WEIGHT = 0.05
REAL_SED_MAX_MODELS = 3
REAL_SED_MIN_MODELS = 1
REAL_SED_TIME_BUFFER_SEC = 10 * 60
REAL_SED_EST_SEC_PER_FILE_PER_MODEL = 0.25
REAL_SED_ZIP_NAME = 'sed-b0-oofteacher-sourcefold-b0v26init-v1.zip'
REAL_SED_DATASET_SLUG = 'bc26-sed-b0-oofteacher-sourcefold-v1'
...
def local_event_boost_logits(scores, alpha=LOCAL_EVENT_ALPHA, topk=LOCAL_EVENT_TOPK):
    ...
    if n_complete_files <= 0:
        return scores.copy()
    view = scores[:n_complete_files * N_WINDOWS].reshape(-1, N_WINDOWS, scores.shape[1]).copy()
    ...
    result = boosted.reshape(-1, scores.shape[1])
    if remainder > 0:
        result = np.vstack([result, scores[n_complete_files * N_WINDOWS:]])
    return result
...
final_scores_smoothed = local_event_boost_logits(final_scores_smoothed)
...
# === Cell: v510 Real SED bundle blend ===
...
# OOF-teacher B0 bundle dataset, emits the same 5-second row IDs, blends probabilities,
# then applies the v517 taxon gate to the combined probabilities.
def _sed_hz_to_mel(hz):
    return 2595.0 * np.log10(1.0 + np.asarray(hz) / 700.0)


def _sed_mel_to_hz(mel):
    return 700.0 * (10.0 ** (np.asarray(mel) / 2595.0) - 1.0)


def _sed_make_mel_filter(sr, n_fft, n_mels, fmin=20.0, fmax=None):
    if fmax is None:
        fmax = sr / 2
    mel_points = np.linspace(_sed_hz_to_mel(fmin), _sed_hz_to_mel(fmax), n_mels + 2)
    hz_points = _sed_mel_to_hz(mel_points)
...
def _sed_find_manifest():
    candidates = [
        Path('/kaggle/input') / REAL_SED_DATASET_SLUG / 'sed_bundle_manifest.json',
        Path('/kaggle/input') / REAL_SED_DATASET_SLUG / 'sed-b0-oofteacher-sourcefold-b0v26init-v1' / 'sed_bundle_manifest.json',
        Path('/kaggle/working/sed_b0_oofteacher_sourcefold_bundle/sed_bundle_manifest.json'),
    ]
    for c in candidates:
        if c.exists():
            return c
    ...
    zip_candidates += list(Path('/kaggle/input').glob(f'**/{REAL_SED_ZIP_NAME}'))
    for z in zip_candidates:
        if z.exists():
            out_dir = Path('/kaggle/working/sed_b0_oofteacher_sourcefold_bundle')
            out_dir.mkdir(parents=True, exist_ok=True)
            manifest = out_dir / 'sed_bundle_manifest.json'
            if not manifest.exists():
                print(f'Extracting real SED bundle from {z} ...')
                with zipfile.ZipFile(z, 'r') as zf:
                    zf.extractall(out_dir)
            if manifest.exists():
                return manifest
            extracted_hits = sorted(out_dir.glob('**/sed_bundle_manifest.json'))
            if extracted_hits:
                print('Real SED extracted manifest: ' + str(extracted_hits[0]))
                return extracted_hits[0]
    return None


def _sed_decode_audio(path, sr, target_samples):
    try:
        raw = subprocess.check_output(['ffmpeg', '-v', 'error', '-i', str(path), '-f', 'f32le', '-ac', '1', '-ar', str(sr), '-'], timeout=90)
        y = np.frombuffer(raw, dtype=np.float32).copy()
    except Exception as exc:
        print(f'WARNING: ffmpeg SED decode failed for {path.name}: {exc}; trying soundfile fallback')
        ...
        if int(file_sr) != int(sr):
            try:
                ...
                print(f'WARNING: SED resample fallback failed for {path.name}: {resample_exc}')
                return None
...
def _sed_extract_context(audio, sr, end_sec, context_sec):
...
def _sed_waveform_to_logmel(wave, audio_cfg, mel_fb):
...
def _sed_select_entries(entries, n_files):
    # Estimate remaining CPU budget and round-robin across bundle members so a
    # time-capped run still includes both v13 and v15 signal.
    budget = max(0.0, time_remaining() - REAL_SED_TIME_BUFFER_SEC)
    if n_files > 0:
        max_by_time = int(budget / max(n_files * REAL_SED_EST_SEC_PER_FILE_PER_MODEL, 1.0))
    else:
        max_by_time = len(entries)
    max_models = min(len(entries), REAL_SED_MAX_MODELS, max_by_time)
    if max_models < REAL_SED_MIN_MODELS:
        print(f'WARNING: skipping real SED blend; insufficient time budget ({time_remaining()/60:.1f} min left)')
        return []
    by_member = defaultdict(list)
    for e in entries:
        by_member[e.get('member', 'bundle')].append(e)
    selected = []
    while len(selected) < max_models:
        added = False
        for member in sorted(by_member):
            if by_member[member] and len(selected) < max_models:
                selected.append(by_member[member].pop(0))
                added = True
        if not added:
            break
    return selected
...
def _sed_infer_bundle(test_paths, target_row_ids):
    manifest_path = _sed_find_manifest()
    if manifest_path is None:
        print('WARNING: real SED bundle dataset not found; using v508 probabilities only')
        return None
    print(f'Real SED manifest: {manifest_path}')
    manifest = json.loads(manifest_path.read_text())
    labels = [str(x) for x in manifest['labels']]
    audio_cfg = manifest['audio_config']
    entries = _sed_select_entries(manifest['models'], len(test_paths))
    if not entries:
        return None
    print(f'Loading {len(entries)}/{len(manifest["models"])} real SED models')
    device = torch.device('cpu')
    torch.set_num_threads(2)
    models = []
    sed_sess_options = None
    if HAS_ONNX:
        sed_sess_options = ort.SessionOptions()
        sed_sess_options.graph_optimization_level = ort.GraphOptimizationLevel.ORT_ENABLE_ALL
        sed_sess_options.intra_op_num_threads = 2
        sed_sess_options.inter_op_num_threads = 1
        sed_sess_options.enable_mem_pattern = False
        sed_sess_options.enable_cpu_mem_arena = False
    for entry in entries:
        ...
        if str(model_path).endswith('.onnx'):
            if not HAS_ONNX:
                print(f'WARNING: skipping ONNX SED model without onnxruntime: {model_path.name}')
                continue
            models.append((entry, 'onnx', ort.InferenceSession(str(model_path), sed_sess_options, providers=['CPUExecutionProvider'])))
        else:
            models.append((entry, 'torchscript', torch.jit.load(str(model_path), map_location=device).eval()))
    if not models:
        print('WARNING: no real SED models could be loaded')
        return None
    weight_sum = sum(float(e.get('weight', 0.0)) for e, _, _ in models)
    if weight_sum <= 0:
        weight_sum = float(len(models))
    sr = int(audio_cfg['sample_rate'])
    context_sec = float(audio_cfg['duration_sec'])
    mel_fb = _sed_make_mel_filter(sr, int(audio_cfg['n_fft']), int(audio_cfg['n_mels']))
    sed_rows = []
    sed_probs = []
    sed_start = time.time()
    for path_i, path in enumerate(test_paths):
        audio = _sed_decode_audio(path, sr, int(round(sr * 60.0)))
        if audio is None:
            continue
        windows = [
            _sed_waveform_to_logmel(torch.from_numpy(_sed_extract_context(audio, sr, end_sec, context_sec).copy()), audio_cfg, mel_fb)
            for end_sec in range(5, 65, 5)
        ]
        x_all = torch.stack(windows)
        file_probs = np.zeros((12, len(labels)), dtype=np.float32)
        with torch.no_grad():
            for entry, kind, model in models:
                pred_chunks = []
                for start in range(0, len(x_all), 4):
                    xb = x_all[start:start + 4]
                    if kind == 'onnx':
                        clip_logits_np = model.run(None, {'logmel': xb.cpu().numpy().astype(np.float32)})[0].astype(np.float32)
                        pred_chunks.append((1.0 / (1.0 + np.exp(-clip_logits_np))).astype(np.float32))
                    else:
                        clip_logits, _frame_logits = model(xb)
                        pred_chunks.append(torch.sigmoid(clip_logits).cpu().numpy().astype(np.float32))
                file_probs += (float(entry.get('weight', 0.0)) / weight_sum) * np.concatenate(pred_chunks, axis=0)
        sed_rows.extend([f'{path.stem}_{end_sec}' for end_sec in range(5, 65, 5)])
        sed_probs.append(file_probs)
        if (path_i + 1) % 10 == 0:
            print(f'  Real SED inferred {path_i+1}/{len(test_paths)} files; remaining={time_remaining()/60:.1f} min')
    if not sed_probs:
        return None
    sed_arr = np.vstack(sed_probs)
    pred = pd.DataFrame(sed_arr, columns=labels)
    pred.insert(0, 'row_id', sed_rows)
    for col in PRIMARY_LABELS:
        if col not in pred.columns:
            pred[col] = 0.0
    pred = pred.set_index('row_id').reindex(target_row_ids, fill_value=0.0).reset_index()
    arr = np.clip(pred[PRIMARY_LABELS].to_numpy(np.float32), 1e-8, 1.0 - 1e-8)
    print(f'Real SED prob range: {arr.min():.6f} to {arr.max():.6f}, mean: {arr.mean():.4f}; runtime {time.time()-sed_start:.1f}s')
    return arr


if REAL_SED_BLEND_WEIGHT > 0:
    try:
        sed_probs_real = _sed_infer_bundle(test_paths, meta_test['row_id'].values)
        if sed_probs_real is not None and sed_probs_real.shape == probs.shape:
            probs = (1.0 - REAL_SED_BLEND_WEIGHT) * probs + REAL_SED_BLEND_WEIGHT * sed_probs_real
            probs = np.clip(probs, 1e-8, 1.0 - 1e-8)
            print(f'Applied OOF-teacher B0 SED bundle blend: weight={REAL_SED_BLEND_WEIGHT}')
        else:
            print('WARNING: real SED blend skipped due to missing/mismatched predictions')
    except Exception as exc:
...
print(f'  Taxon max gate: floor={TAXON_MAX_GATE_FLOOR}, alpha={TAXON_MAX_GATE_ALPHA}')
print(f'  Post: v508 immediate temporal/logit local-event axis + source-fold OOF-teacher B0 sidecar blend weight {REAL_SED_BLEND_WEIGHT}, then v517 taxon max gate')
