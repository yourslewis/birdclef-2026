...
# v607 repair: persist ProtoSSM probabilities for the downstream rank-blend cell.
proto_probs_flat = (1.0 / (1.0 + np.exp(-np.clip(proto_scores_flat, -50, 50)))).astype(np.float32)
proto_sub = pd.DataFrame(np.clip(proto_probs_flat, 0.0, 1.0), columns=PRIMARY_LABELS)
proto_sub.insert(0, "row_id", meta_te["row_id"].values)
proto_sub.to_csv("submission_protossm.csv", index=False)
print("submission_protossm.csv saved", proto_sub.shape, float(proto_probs_flat.min()), float(proto_probs_flat.max()))
