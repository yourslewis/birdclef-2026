...
# %% markdown
# ### v596 streaming HGNetV2-B0 distilled sidecar
#
...
if not HGNET_WEIGHT_DIRS:
    raise FileNotFoundError('v596 HGNet weights not found: expected best_model_fold0.pt under /kaggle/input')
...
print(f'v596 streaming HGNet sidecar using weights from {HGNET_WEIGHT_DIR}; model_device={HGNET_DEVICE}; preprocess_device={HGNET_PREPROCESS_DEVICE}')
...
print(f'v596 streaming HGNet target rows={len(_hgnet_row_ids)}, cols={len(_hgnet_cols)}, dry_run={_hgnet_is_dry_run}')
...
def _load_audio_once(audio_id: str):
    candidates = [BASE / 'test_soundscapes' / f'{audio_id}.ogg', BASE / 'train_soundscapes' / f'{audio_id}.ogg']
    path = next((p for p in candidates if p.exists()), None)
    if path is None:
        raise FileNotFoundError(f'HGNet audio file not found for {audio_id}; checked {candidates}')
    ...
    if int(sr) != HGNET_SR:
        tw = torch.tensor(wav, dtype=torch.float32)
        wav = torchaudio.functional.resample(tw, int(sr), HGNET_SR).numpy().astype(np.float32)
        sr = HGNET_SR
    return wav, int(sr)

def _segment_from_wav(wav: np.ndarray, end_sec: int) -> np.ndarray:
    start = max(0, int((end_sec - HGNET_SEGMENT_SEC) * HGNET_SR))
    stop = int(end_sec * HGNET_SR)
    seg = wav[start:stop]
    target_len = HGNET_SR * HGNET_SEGMENT_SEC
...
# Group rows by audio id so the hidden run only keeps one 60s waveform and one
# small mel batch in memory at a time.  v594 materialized all hidden segments,
# all fold logits, and an all-file audio cache, which caused hidden RAM failure.
_hgnet_groups = {}
for idx, rid in enumerate(_hgnet_row_ids):
    aid, end_sec = _parse_row_id_for_audio(rid)
    _hgnet_groups.setdefault(aid, []).append((idx, end_sec))
print(f'v596 streaming HGNet audio groups={len(_hgnet_groups)}, initial RSS={_rss_mb():.1f} MB')
...
_hgnet_probs_sum = np.zeros((len(_hgnet_row_ids), len(_hgnet_cols)), dtype=np.float32)
with torch.no_grad():
    for fold in range(4):
        print(f'v596 HGNet fold {fold} start RSS={_rss_mb():.1f} MB')
        ...
        fold_max = float('-inf')
        fold_rows = 0
        for audio_i, (aid, row_items) in enumerate(_hgnet_groups.items()):
            wav, sr = _load_audio_once(aid)
            idxs = [idx for idx, _ in row_items]
            segs = [_segment_from_wav(wav, end_sec) for _, end_sec in row_items]
            for start in range(0, len(segs), HGNET_BATCH):
                batch_idxs = idxs[start:start + HGNET_BATCH]
                batch_arr = np.stack(segs[start:start + HGNET_BATCH]).astype(np.float32, copy=False)
                batch = torch.from_numpy(batch_arr).to(HGNET_PREPROCESS_DEVICE)
                lms = _lms(batch).to(HGNET_DEVICE, non_blocking=True)
                logits = model(lms).detach().float().cpu().numpy()
                probs = (1.0 / (1.0 + np.exp(-logits))).astype(np.float32)
                _hgnet_probs_sum[batch_idxs, :] += probs / 4.0
                fold_min = min(fold_min, float(np.nanmin(probs)))
                fold_max = max(fold_max, float(np.nanmax(probs)))
                fold_rows += len(batch_idxs)
                del batch_arr, batch, lms, logits, probs
            del wav, segs
            if audio_i and audio_i % 50 == 0:
                print(f'v596 HGNet fold {fold}: processed audio_groups={audio_i}/{len(_hgnet_groups)}, RSS={_rss_mb():.1f} MB')
        print(f'v596 HGNet fold {fold}: rows={fold_rows}, prob_min={fold_min:.6f}, prob_max={fold_max:.6f}, RSS={_rss_mb():.1f} MB')
        del model, state
        gc.collect()
        if torch.cuda.is_available():
            torch.cuda.empty_cache()

_hgnet_probs = _hgnet_probs_sum.astype(np.float32)
...
print(f'v596 wrote {HGNET_OUT_CSV}: shape={_hgnet_sub.shape}, min={float(_hgnet_probs.min()):.6f}, max={float(_hgnet_probs.max()):.6f}, RSS={_rss_mb():.1f} MB')
del _hgnet_groups, _hgnet_probs_sum, _hgnet_probs, _hgnet_sub, _lms
gc.collect()
if torch.cuda.is_available():
    torch.cuda.empty_cache()
...
print(f"Executing v596 streaming 3-way rank blend: Proto={proto_w:.4f} SED={sed_w:.4f} HGNet={HGNET_RANK_WEIGHT:.4f}")
