...
def morphological_smoothing(predictions, size=3):
    # predictions: shape (time_steps, num_classes)
    # Applying median filter along the time axis (axis 0) 
    # rejects 1-frame spikes (noise) while preserving solid 2-frame or 3-frame calls perfectly.
    return median_filter(predictions, size=(size, 1))
