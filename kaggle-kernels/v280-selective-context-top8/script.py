#!/usr/bin/env python3
"""
BirdCLEF 2026 — v280 Selective Context Top8 Kernel
=============================================================================
...
- Target behavior: improve over the current 0.923 LB anchor by combining v111/v118
  strengths plus immediate-neighbor temporal smoothing and strict top-8 file-context boosting.
"""
...
print('V280: immediate-only temporal smoothing + selective top-8 file-context boost')

# === Cell: Settings ===
# All high-impact knobs are centralized here so experiments can be audited.
# v280 keeps the known-strong v111/v118 family of settings, especially:
...
def file_context_boost(probs, alpha=FILE_CONTEXT_ALPHA, topk=FILE_CONTEXT_TOPK):
    ...
    if n_complete_files > 0:
        ...
        if topk is not None and 0 < topk < probs.shape[1]:
            ...
            # species per soundscape. Top-8 is a strict presence-style gate that
            # should limit long-tail false-positive lifts while keeping repeated
            # species context for the highest-confidence classes.
