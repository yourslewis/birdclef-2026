...
# Soft Pseudo-Label configuration
# We assume the teacher predictions (probabilities between 0 and 1) are saved in a CSV or tensor
USE_SOFT_PSEUDO_LABELS = True
SOFT_LABEL_WEIGHT = 0.5  # Weight of the distillation loss vs true hard labels
...
class DistillationLoss(nn.Module):
    ...
    def forward(self, student_logits, true_labels, teacher_probs):
        # Loss against ground truth
        hard_loss = self.bce(student_logits, true_labels)
        # Loss against teacher's soft probabilities
        soft_loss = self.bce(student_logits, teacher_probs)
