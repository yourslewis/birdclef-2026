...
print("Executing v546 conservative 2-way rank blend (60% Proto / 40% SED) + 5% train-audio-head on trained classes...")
...
# Source-clean v546 addition: a tiny public linear head trained on train_audio
# Perch embeddings.  This follows the audited Henry/Raunak public fork but
# hard-fails instead of silently skipping so a bad dataset mount cannot submit
# the plain public946 fallback.  The blend is class-masked by trained_mask.
import glob
head_candidates = sorted(glob.glob("/kaggle/input/**/head_weights_train_audio.npz", recursive=True))
print("v546 train-audio-head candidates:", head_candidates[:5])
if not head_candidates:
    raise FileNotFoundError("head_weights_train_audio.npz not found under /kaggle/input")
...
head_mask = head_weights["trained_mask"].astype(bool).reshape(1, -1) if "trained_mask" in head_weights.files else np.ones((1, len(cols)), dtype=bool)
if "emb_te" not in globals():
    raise RuntimeError("emb_te is unavailable for train-audio-head inference")
if emb_te.ndim != 2:
    raise RuntimeError(f"emb_te must be 2D, got shape {emb_te.shape}")
if emb_te.shape[0] != len(df_proto):
    raise RuntimeError(f"head row mismatch: emb_te rows={emb_te.shape[0]} submission rows={len(df_proto)}")
if emb_te.shape[1] != head_W.shape[1]:
    raise RuntimeError(f"head dim mismatch: emb_te={emb_te.shape[1]} W={head_W.shape[1]}")
if head_W.shape[0] != len(cols) or head_b.shape[0] != len(cols) or head_mask.shape[1] != len(cols):
    raise RuntimeError(f"head class mismatch: W={head_W.shape} b={head_b.shape} mask={head_mask.shape} cols={len(cols)}")
head_logits = emb_te.astype(np.float32) @ head_W.T + head_b[None, :]
head_probs = 1.0 / (1.0 + np.exp(-np.clip(head_logits, -30, 30)))
head_rank = pd.DataFrame(np.clip(head_probs, EPS, 1.0 - EPS)).rank(axis=0, pct=True).to_numpy(np.float32)
if not np.isfinite(head_rank).all():
    raise ValueError("train-audio-head predictions contain NaN/Inf")
head_weight = HEAD_RANK_BLEND * head_mask.astype(np.float32)
pred = (1.0 - head_weight) * pred + head_weight * head_rank
print(
    f"v546 train-audio-head used: {head_candidates[0]} weight={HEAD_RANK_BLEND} "
    f"trained_classes={int(head_mask.sum())}/{len(cols)} "
    f"head_prob_range=({float(head_probs.min()):.6f},{float(head_probs.max()):.6f}) "
    f"head_prob_mean={float(head_probs.mean()):.6f}"
)

sub = df_proto.copy()
sub[cols] = np.clip(pred, 0.0, 1.0).astype(np.float32)
sub.to_csv("submission_train_audio_head.csv", index=False)
print(f"v546 wrote submission_train_audio_head.csv {sub.shape}")
...
print("v546 train-audio-head blend and post-processing complete. Ready for submission!")
